...
export async function POST(req: NextRequest) {
  ...
  // Fall back to the live caller-ID from the Retell envelope if the LLM
  // forgot to pass phone — the lookup should never fail on that.
  const phone: string = body?.phone || callerNumber || '';
  if (!tenantId || !phone) {
