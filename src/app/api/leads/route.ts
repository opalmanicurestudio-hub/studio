...
export async function POST(req: NextRequest) {
  ...
  const tools = Array.isArray(b.tools) ? b.tools.map((x: any) => String(x).slice(0, 20)).slice(0, 15) : [];
  ...
  await ref.set({ id: ref.id, name, email, phone: phone || null, business, type, size: size || null, currently: currently || null, note: note || null, tools, source: String(b.source || 'site').slice(0, 40), status: 'new', createdAt: at });

  const to = process.env.LEADS_NOTIFY_EMAIL;
  if (to && process.env.RESEND_API_KEY) {
    const lines = [`${name} — ${business} (${type}${size ? `, ${size}` : ''})`, `Email: ${email}${phone ? ` · Phone: ${phone}` : ''}`, currently ? `Currently using: ${currently}` : '', note ? `Note: ${note}` : '', tools.length ? `Tools: ${tools.join(', ')}` : ''].filter(Boolean);
    try {
