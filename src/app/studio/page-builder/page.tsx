...
// ─── Types ────────────────────────────────────────────────────────────────────
type FieldType = 'text' | 'textarea' | 'toggle' | 'select' | 'image' | 'color' | 'range' | 'image-array' | 'social-links' | 'policy-list' | 'tag-list';
...
interface SectionLayoutOption {
  id: string;
  label: string;
  preview: string; // ASCII or emoji preview hint
}
...
const SOCIAL_PLATFORMS = [
  ...
  { id: 'snapchat',   label: 'Snapchat',   icon: MessageCircle, placeholder: 'https://snapchat.com/yourstudio',  color: '#FFFC00' },
...
const SECTION_DEFS: Record<SectionType, SectionDef> = {
  ...
  hero: {
    ...
    fields: [
      ...
      { k: 'cta2Action',   t: 'select',   l: 'Secondary action',       d: 'scroll-contact', opts: ['booking', 'scroll-contact', 'scroll-services', 'url'] },
  ...
  beforeafter: {
    ...
    fields: [
      ...
      { k: 'pairs',       t: 'image-array', l: 'Before/After pairs', d: []                           },
      { k: 'sliderColor', t: 'color',       l: 'Slider handle color', d: ''                          },
...
/** Image array editor (gallery / before-after) */
const ImageArrayEditor = ({ value, onChange, maxImages = 50 }: {
  ...
  const add = () => onChange([...images, { id: generateId(), url: '', caption: '', category: '' }]);

  const update = (id: string, field: keyof GalleryImage, val: string) =>
    onChange(images.map(img => img.id === id ? { ...img, [field]: val } : img));
      ...
      {images.length < maxImages && (
        <div>
          ...
          <ImageUpload
            initialImage=""
            onImageUploaded={(url) => {
              onChange([...images, { id: generateId(), url, caption: '', category: '' }]);
            }}
          />
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1.5">
        {tags.map(tag => (
          <span key={tag} className="flex items-center gap-1 px-2 py-1 bg-primary/10 text-primary rounded-lg text-[10px] font-bold">
            {tag}
            <button onClick={() => onChange(tags.filter(t => t !== tag))} className="ml-0.5">
        ...
        <div className="flex h-full">
            ...
            {selectedSection && activePanel === 'sections' ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-6 p-8">
                ...
                <div className="text-center space-y-1">
                  <p className="text-sm font-black uppercase tracking-tight text-slate-900">Style preview</p>
                </div>
          ...
          <div className="flex-1 h-full flex-col bg-slate-100 hidden lg:flex">
            ...
            <div className="flex-1 flex items-center justify-center p-6 overflow-hidden">
              {previewUrl ? (
                <div className={cn('h-full transition-all duration-300', previewMode === 'mobile' ? 'w-[390px]' : 'w-full')}>
                  <iframe
                    key={previewKey}
                    ref={previewRef}
                    src={previewUrl}
                    className={cn('w-full h-full border-0 bg-white',
                      previewMode === 'mobile' ? 'rounded-[2.5rem] shadow-2xl ring-8 ring-slate-800' : 'rounded-2xl shadow-xl')}
