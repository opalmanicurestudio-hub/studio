...
const generateICS = (event: any, ticket: any): string => {
  ...
  return [
    'BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Opal Studio//Event Ticket//EN',
    'CALSCALE:GREGORIAN','METHOD:PUBLISH','BEGIN:VEVENT',
    `UID:${ticket.id}@opal-studio`,`DTSTART:${fmt(start)}`,`DTEND:${fmt(end)}`,
...
const downloadICS = (event: any, ticket: any) => {
  const blob = new Blob([generateICS(event, ticket)], { type: 'text/calendar;charset=utf-8' });
...
type GuestInfo = { name: string; email: string; phone: string; };
...
export default function EventPublicPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ─── FIX: count tickets from eventTickets collection ──────────────
        ...
        if (stripeSessionId) {
          ...
          if (!existing.empty) {
            setTicket({ id: existing.docs[0].id, ...existing.docs[0].data() });
        ...
        if (evData.status === 'closed')    { setErrorMsg('Registration is closed.'); setPageState('closed'); return; }
  ...
  const handleFreeRsvp = async () => {
    ...
    try {
      ...
      // ─── FIX: write to eventTickets (NOT events/{id}/guestOrders) ────────
      // This is the collection the manifest page reads from.
      const newTicket = {
        id,
        eventId,
        tenantId,
      ...
      // Primary write: eventTickets (manifest reads this)
      await addDoc(collection(db, `tenants/${tenantId}/eventTickets`), newTicket);

      // Also increment the event's guestCount for quick display on the event card
      try {
        await updateDoc(doc(db, `tenants/${tenantId}/studioEvents`, eventId), {
    ...
    <div className="min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      <div className="relative overflow-hidden" style={{ background: primaryHex, minHeight: 260 }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: `radial-gradient(ellipse at 30% 50%, rgba(255,255,255,0.12) 0%, transparent 70%), radial-gradient(ellipse at 80% 20%, rgba(0,0,0,0.15) 0%, transparent 60%)` }} />
        <div className="relative z-10 max-w-2xl mx-auto px-6 py-10">
          ...
          {invitedGuest && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-[9px] font-black uppercase tracking-[0.3em]" style={{ color: textOn }}>You've been personally invited</p>
            </motion.div>
          )}
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <AnimatePresence mode="wait">
          ...
          {pageState === 'event' && event && (
            <motion.div key="event" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
              {event.description && (
              ...
              <div className="bg-white rounded-3xl border-2 border-slate-100 p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-black text-xl text-slate-900">{ticketConfig.ticketName || (isPaid ? 'General Admission' : 'Free Event')}</p>
                    <p className="text-slate-500 text-sm font-bold mt-0.5">{isPaid ? `$${ticketConfig.price?.toFixed(2)} per person` : 'Free to attend'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-black" style={{ color: primaryHex }}>{isPaid ? `$${ticketConfig.price?.toFixed(2)}` : 'Free'}</p>
                    {capacity && <p className="text-[9px] font-bold text-slate-400 mt-0.5">{Math.max(0, capacity - ticketCount)} of {capacity} left</p>}
                  </div>
                </div>
                <motion.button whileTap={{ scale: 0.98 }} onClick={() => setPageState('form')}
          ...
          {pageState === 'form' && (
            <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
              ...
              <div className="bg-white rounded-3xl border-2 border-slate-100 p-6 space-y-5">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400 mb-1">{isPaid ? 'Checkout' : 'Complete your RSVP'}</p>
                ...
                {invitedGuest && (
                  <div className="flex items-center gap-2 p-3 rounded-2xl bg-violet-50 border-2 border-violet-100">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-violet-700">Invited guest — details pre-filled</p>
                  </div>
                )}
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400">Full Name</label>
                  <input value={guest.name} onChange={e => setGuest(g => ({ ...g, name: e.target.value }))} placeholder="Your full name"
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="email" value={guest.email} onChange={e => setGuest(g => ({ ...g, email: e.target.value }))} placeholder="your@email.com"
                    className="w-full h-12 rounded-2xl border-2 border-slate-100 px-4 text-sm font-bold text-slate-800 outline-none focus:border-slate-300 placeholder:text-slate-300" />
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="tel" value={guest.phone} onChange={e => setGuest(g => ({ ...g, phone: e.target.value }))} placeholder="(555) 000-0000"
                    className="w-full h-12 rounded-2xl border-2 border-slate-100 px-4 text-sm font-bold text-slate-800 outline-none focus:border-slate-300 placeholder:text-slate-300" />
                </div>
                {isPaid && (
                ...
                {isPaid && <p className="text-center text-[9px] font-bold text-slate-400">Secured by Stripe · Your card details are never stored</p>}
              </div>
            </motion.div>
          )}
          ...
          {pageState === 'confirmed' && ticket && (
            <motion.div key="conf" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              <div className="text-center space-y-2">
                ...
                <h2 className="text-3xl font-black text-slate-900">{isPaid ? "You're in!" : "RSVP Confirmed!"}</h2>
                <p className="text-slate-500 font-bold">See you there{invitedGuest ? `, ${invitedGuest.name?.split(' ')[0]}` : ''}!</p>
              </div>
              <TicketCard ticket={ticket} event={event} onDownloadCal={() => downloadICS(event, ticket)} />
              <div className="text-center space-y-2">
          ...
          {pageState === 'sold_out' && (
            <motion.div key="sold" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 space-y-4">
          ...
          {(pageState === 'closed' || pageState === 'error') && (
            <motion.div key="closed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 space-y-4">
              ...
              <h2 className="text-2xl font-black text-slate-900">{pageState === 'error' ? 'Something went wrong' : 'Not Available'}</h2>
