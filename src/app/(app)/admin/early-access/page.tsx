...
import { HqShell } from '@/components/hq/hq';
...
export default function EarlyAccessPage() {
  ...
  if (err) return (<HqShell><p className="m-6 rounded-2xl bg-red-50 p-4 text-sm text-red-800">{err}</p></HqShell>);
  if (!data) return (<HqShell><div className="flex justify-center p-16"><Loader className="h-6 w-6 animate-spin text-stone-400" /></div></HqShell>);

  const counts = STATUSES.map((s) => [s, data.leads.filter((l: any) => (l.status || 'new') === s).length] as const);
  return (
    <HqShell title="Early access" sub="Requests from the website, and your invites.">
        <div>
          <h1 className="text-3xl font-light tracking-tight">Early access</h1>
          <p className="text-sm text-stone-500">Sign-up is <span className="font-bold">{data.signupOpen ? 'open to everyone' : 'by invite only'}</span>. Requests from the website land here.</p>
        </div>
        ...
        <section className="space-y-2">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400">Requests</p>
          {data.leads.length === 0 && <p className="rounded-[1.75rem] border border-dashed border-stone-300 p-6 text-center text-sm text-stone-500">No requests yet. They’ll appear here from /request-access.</p>}
          {data.leads.map((l: any) => (
            <article key={l.id} className="space-y-2 glass rounded-[1.75rem] border border-white/70 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold">{l.business} <span className="font-medium text-stone-500">· {l.type}{l.size ? ` · ${l.size}` : ''}</span></p>
                  <p className="text-sm text-stone-600">{l.name} · <a href={`mailto:${l.email}`} className="underline">{l.email}</a>{l.phone ? ` · ${l.phone}` : ''}</p>
                  <p className="text-xs text-stone-400">{when(l.createdAt)}{l.currently ? ` · uses ${l.currently}` : ''}{l.inviteCode ? ` · invite ${l.inviteCode}` : ''}</p>
                </div>
              ...
              {Array.isArray(l.tools) && l.tools.length > 0 && <p className="flex flex-wrap gap-1">{l.tools.map((t: string) => <span key={t} className="rounded-full bg-white/70 px-2 py-0.5 text-[11px]">{TOOL_BY_ID[t as ToolId]?.emoji} {TOOL_BY_ID[t as ToolId]?.name || t}</span>)}</p>}
              {l.note && <p className="rounded-2xl bg-white/60 p-3 text-sm text-stone-700">“{l.note}”</p>}
              {(l.status || 'new') !== 'onboarded' && (
                <div className="flex flex-wrap gap-2">
                  <button type="button" disabled={!!busy} onClick={() => invite({ leadId: l.id, send: true }, l.id)} className="h-9 rounded-xl bg-stone-900 px-4 text-xs font-bold text-white disabled:opacity-50">{busy === l.id ? '…' : l.inviteCode ? 'Send a new invite' : 'Invite & email'}</button>
        ...
        <section className="space-y-2 glass rounded-[1.75rem] border border-white/70 p-4">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400">Invite someone directly</p>
          ...
          <button type="button" disabled={!!busy} onClick={() => invite({ email: manual.email, business: manual.business, maxUses: Number(manual.uses) || 1, send: !!manual.email }, 'manual')} className="h-9 rounded-xl bg-stone-900 px-4 text-xs font-bold text-white disabled:opacity-50">{busy === 'manual' ? '…' : manual.email ? 'Create & email' : 'Create code'}</button>
          <p className="text-[11px] text-stone-500">A code with more than one use is handy for an academy class or a suite building.</p>
        </section>

        <section className="space-y-2">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400">Invites</p>
          {data.invites.map((i: any) => (
            <div key={i.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/70 bg-white/60 px-3 py-2 text-sm">
              <span className="min-w-0 truncate"><span className="font-mono font-semibold">{i.code}</span> · {i.business || i.email || 'open code'} · {i.uses || 0}/{i.maxUses || 1} used · {when(i.createdAt)}</span>
