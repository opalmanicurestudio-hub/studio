          ...
          <>
            <section className="space-y-2 rounded-3xl border-2 border-slate-200 bg-white p-4">
              ...
              {d.settings.map((s: any) => (
                <div key={s.key} className="flex items-start gap-3 text-sm"><span className={s.ok ? 'text-emerald-600' : 'text-red-600'}>{s.ok ? '✓' : '✕'}</span><span className="min-w-0 flex-1"><span className="font-bold">{s.key}</span> <span className="text-slate-500">— {s.note}</span></span></div>
              ))}
            </section>

            <section className="space-y-2 rounded-3xl border-2 border-slate-200 bg-white p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Daily jobs</p>
              {d.jobs.map((j: any) => (
                <div key={j.name} className={`flex items-center justify-between gap-3 rounded-2xl px-3 py-2 text-sm ${j.late ? 'bg-red-50 text-red-900' : 'bg-slate-50'}`}>
              ...
              <p className="text-[11px] text-slate-500">Jobs start recording once this version is live — “never recorded” is expected until each has run once.</p>
            </section>

            <section className="space-y-3 rounded-3xl border-2 border-slate-200 bg-white p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Delivery · last 7 days · {d.tenants} businesses</p>
              <div className="grid grid-cols-2 gap-2">
                {(['email', 'sms'] as const).map((ch) => (
                  <div key={ch} className="rounded-2xl bg-slate-50 p-3"><p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">{ch === 'sms' ? 'Texts' : 'Emails'}</p><p className="text-2xl font-black">{rate(d.delivery[ch])}</p><p className="text-[12px] text-slate-500">{d.delivery[ch].sent} sent · {d.delivery[ch].failed} failed{d.delivery[ch].other ? ` · ${d.delivery[ch].other} held` : ''}</p></div>
                ))}
              </div>
            ...
            <section className="space-y-2 rounded-3xl border-2 border-slate-200 bg-white p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">App versions</p>
              ...
              <p className="text-[12px] text-slate-500">{d.openTickets} help request{d.openTickets === 1 ? '' : 's'} waiting · <Link href="/admin/support" className="underline">Help inbox</Link></p>
            </section>
          </>
        )}
      </main>
