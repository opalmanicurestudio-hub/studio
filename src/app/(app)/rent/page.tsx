...
import {
  createBookingHold,
  confirmDayPassAtDesk,
  releaseBookingHold,
  syncDayPassAvailability,
  DayPassOverrideConflictError,
...
// ── Day passes ───────────────────────────────────────────────────────────────
// A spot rental: somebody wants a chair on Tuesday. Deliberately NOT a lease —
// its own object, its own rate, no renewal, no late-fee policy, no invoice
// cycle. That is exactly why none of the rent crons need to know it exists.
//
// The conflict check runs inside a transaction in the service layer, against
// both the occupying lease's schedule and any other pass on that booth, with
// the booth's turnover buffer applied. Two people cannot end up in one chair
// even if you and the front desk book the same slot at the same moment.
//
// Confirming also grants the pass-holder AVAILABILITY for that window, using
// the same date override an accepted swap writes. Without that step they would
// be paying for a station nobody can book them into.
function DayPassesCard({ tenantId, firestore, booths, leases, renters }: { tenantId: string; firestore: any; booths: any[]; leases: any[]; renters: any[] }) {
  ...
  const bookingsRef = useMemoFirebase(
    () => (firestore && tenantId ? collection(firestore, BOOTH_RENTAL_COLLECTIONS.bookings(tenantId)) : null),
    [firestore, tenantId]
  );
  const { data: bookings } = useCollection<any>(bookingsRef);

  const dayUseBooths = useMemo(() => (booths || []).filter((b: any) => b.dayUseEnabled), [booths]);
  const booth = dayUseBooths.find((b: any) => b.id === boothId) || null;
  const todayIso = new Date().toISOString().slice(0, 10);

  const upcoming = useMemo(() => (bookings || [])
    .filter((b: any) => b.status !== 'cancelled' && String(b.startAt || '').slice(0, 10) >= todayIso)
    .sort((a: any, b: any) => String(a.startAt).localeCompare(String(b.startAt)))
    .slice(0, 12), [bookings, todayIso]);

  const startAt = date ? `${date}T${startTime}:00` : '';
  const endAt = date ? `${date}T${endTime}:00` : '';
  const totalCents = booth && startAt && endAt && endTime > startTime
    ? computeBookingTotalCents(booth, { startAt, endAt }, rateType) : 0;

  const nameOf = (id: string) => {
    const r = (renters || []).find((x: any) => x.id === id);
    return r ? `${r.firstName || ''} ${r.lastName || ''}`.trim() || 'Renter' : 'Renter';
  };
  const boothName = (id: string) => (booths || []).find((b: any) => b.id === id)?.name || 'Booth';

  const reset = () => {
    setOpen(false); setBoothId(''); setRenterId(''); setDate('');
    setStartTime('09:00'); setEndTime('17:00'); setRateType('daily'); setErr('');
  };

  const book = async () => {
    if (!booth || !renterId || !date || endTime <= startTime) {
      setErr('Pick a booth, a person, a date and a window that runs forwards.');
      return;
    }
    setBusy('book'); setErr(''); setNote('');
    try {
      const occupyingLease = (leases || []).find((l: any) =>
        l.boothId === boothId && ['active', 'on_leave'].includes(String(l.status))) || null;
      const held = await createBookingHold(firestore, {
        tenantId,
      ...
      await confirmDayPassAtDesk(firestore, { tenantId, bookingId: held.bookingId, paid });
      const sync = await syncDayPassAvailability(firestore, tenantId,
        { id: held.bookingId, renterId, startAt, endAt }, true);
      setNote(sync.applied
        ? `${nameOf(renterId)} is bookable ${startTime}–${endTime} that day.`
        : `Pass booked. They keep the chair, but nothing changed in the booking system — ${sync.reason}.`);
      reset();
    } catch (e: any) {
      if (e instanceof DayPassOverrideConflictError) setErr(e.message);
      else if (e instanceof BookingConflictError || String(e?.name) === 'BookingConflictError') {
        setErr('That booth is already taken for part of that window.');
      } else {
        console.error('day pass failed', e);
  ...
  const cancel = async (b: any) => {
    setBusy(b.id); setErr(''); setNote('');
    try {
      await syncDayPassAvailability(firestore, tenantId, b, false).catch(() => null);
      await releaseBookingHold(firestore, tenantId, b.id, 'cancelled_by_owner');
    } catch (e) {
      console.error('cancel day pass failed', e);
      setErr('Could not cancel that.');
    }
    setBusy('');
  };
  ...
  return (
    <Card className="rounded-[2rem] border-2">
      <CardHeader className="p-5 pb-2">
        <CardTitle className="text-[11px] font-black uppercase tracking-widest">Day passes</CardTitle>
        <p className="text-[11px] font-bold text-slate-500">
          A chair for a day, at its own rate. Not a lease — nothing renews, nothing invoices, no late fees.
        </p>
      </CardHeader>
      <CardContent className="space-y-3 p-5 pt-2">
        {dayUseBooths.length === 0 ? (
          <p className="text-[11px] font-bold text-slate-500">
            No booth has day use switched on yet. Turn it on for a station in Booths and set its rate, then passes can be sold here.
          </p>
        ) : !open ? (
          <button onClick={() => setOpen(true)}
            className="w-full rounded-2xl border-2 border-dashed p-4 text-left">
            <span className="block text-[12px] font-black text-slate-900">Sell a day pass</span>
            <span className="block text-[11px] font-bold text-slate-500">Pick a station, a person and a window.</span>
          </button>
        ) : (
          <div className="space-y-3 rounded-2xl border-2 p-4">
            <div className="space-y-1">
              <label htmlFor="dp-booth" className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Station</label>
              <select id="dp-booth" value={boothId} onChange={(e) => setBoothId(e.target.value)}
                className="w-full rounded-xl border-2 px-3 py-2 text-sm font-bold">
                <option value="">Choose…</option>
                {dayUseBooths.map((b: any) => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label htmlFor="dp-renter" className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Who</label>
              <select id="dp-renter" value={renterId} onChange={(e) => setRenterId(e.target.value)}
                className="w-full rounded-xl border-2 px-3 py-2 text-sm font-bold">
                <option value="">Choose…</option>
                {(renters || []).map((r: any) => (
                  <option key={r.id} value={r.id}>{`${r.firstName || ''} ${r.lastName || ''}`.trim() || 'Renter'}</option>
                ))}
              </select>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div className="space-y-1">
                <label htmlFor="dp-date" className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Date</label>
                <input id="dp-date" type="date" value={date} min={todayIso} onChange={(e) => setDate(e.target.value)}
                  className="w-full rounded-xl border-2 px-2 py-2 text-sm font-bold" />
              </div>
              <div className="space-y-1">
                <label htmlFor="dp-start" className="block text-[10px] font-black uppercase tracking-widest text-slate-400">From</label>
                <input id="dp-start" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                  className="w-full rounded-xl border-2 px-2 py-2 text-sm font-bold" />
              </div>
              <div className="space-y-1">
                <label htmlFor="dp-end" className="block text-[10px] font-black uppercase tracking-widest text-slate-400">To</label>
                <input id="dp-end" type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)}
                  className="w-full rounded-xl border-2 px-2 py-2 text-sm font-bold" />
              </div>
            </div>
            <div className="flex gap-2">
              {(['daily', 'hourly'] as const).map((k) => (
                <button key={k} onClick={() => setRateType(k)}
                  className={cn('flex-1 rounded-xl border-2 py-2 text-[11px] font-black uppercase tracking-widest',
                    rateType === k ? 'bg-slate-900 text-white' : 'bg-white text-slate-600')}>
                  {k}
                </button>
              ))}
            </div>
            <button onClick={() => setPaid(!paid)}
              ...
              <span className="text-[12px] font-black text-slate-900">Paid at the desk</span>
            ...
            <p className="text-[12px] font-black text-slate-900">
              {totalCents > 0 ? formatCents(totalCents) : 'Set a rate on the station to price this.'}
            </p>
            ...
            <div className="flex gap-2">
              <button onClick={book} disabled={!!busy}
                ...
                {busy === 'book' ? 'Booking…' : 'Book it'}
              </button>
              <button onClick={reset}
                className="rounded-2xl bg-slate-100 px-5 py-3 text-[11px] font-black uppercase tracking-widest text-slate-600">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          {upcoming.length === 0 ? (
            <p className="text-[11px] font-bold text-slate-500">No passes booked.</p>
          ) : upcoming.map((b: any) => (
            <div key={b.id} className="flex items-center justify-between gap-3 rounded-2xl border-2 p-3">
              <div className="min-w-0">
                <p className="text-[12px] font-black text-slate-900 truncate">
                  {nameOf(b.renterId)} · {boothName(b.boothId)}
                </p>
                <p className="text-[11px] font-bold text-slate-500 mt-0.5">
                  {String(b.startAt).slice(0, 10)} · {String(b.startAt).slice(11, 16)}–{String(b.endAt).slice(11, 16)} · {formatCents(b.totalCents || 0)}
                  {b.paymentStatus === 'paid' ? '' : ' · owes'}
                </p>
              </div>
              <button onClick={() => cancel(b)} disabled={!!busy}
                className="shrink-0 rounded-xl bg-slate-100 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 disabled:opacity-50">
                {busy === b.id ? '…' : 'Cancel'}
              </button>
            </div>
          ))}
...
export default function RentRollPage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      ...
      {tenantId && <DayPassesCard tenantId={tenantId} firestore={firestore} booths={(booths || []) as any[]} leases={(leases || []) as any[]} renters={(renters || []) as any[]} />}
