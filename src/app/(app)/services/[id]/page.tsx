...
import { 
    ArrowLeft, Clock, Sparkles, Hammer, BarChart, Users, TrendingUp, MapPin, 
    Link as LinkIcon, Target, Zap, Briefcase, Calendar as CalendarIcon, Scale,
    ArrowRight, Loader, Pencil,
} from 'lucide-react';
...
// ── KEY CHANGE: wire up the Modify button with ServiceFormSheet ──
import { EditServiceDialog } from '@/components/services/ServiceFormSheet';

const ProfitAnalysisCard = ({ service, tmhr, staff, pricingTiers, taxBurden }: { 
    service: Service; tmhr: number; staff: Staff[]; pricingTiers: PricingTier[]; taxBurden: number 
}) => {
    ...
    const { materialCost, timeCost } = useMemo(() => {
        ...
        const materialCost = (service.products || []).reduce((acc, p) => {
            ...
            let costPerUse = product.costPerUnit || 0;
    ...
    const tierAnalysis = useMemo(() => {
        return pricingTiers.sort((a,b) => a.rank - b.rank).map(tier => {
            ...
            const staffAnalysis = relevantStaff.map(member => {
                ...
                return { id: member.id, name: member.name, avatarUrl: member.avatarUrl, payStructure: member.payStructure, studioNet, margin };
            });
            return { ...tier, price, duration, staffAnalysis };
        });
        ...
        <Card className="lg:sticky lg:top-24 border-4 rounded-[2.5rem] shadow-2xl shadow-primary/5 overflow-hidden">
            <CardHeader className="p-6 sm:p-8 pb-4 border-b bg-muted/5 text-left">
                <CardTitle className="text-[10px] font-black uppercase tracking-[0.25em] text-primary flex items-center gap-2">
                    <Sparkles className="w-3 h-3" />Yield Engine
                </CardTitle>
                    ...
                    <div key={tier.id} className="space-y-3">
                        ...
                        <div className="grid gap-2">
                            {tier.staffAnalysis.length > 0 ? tier.staffAnalysis.map(sa => (
                                <div key={sa.id} className={cn("p-3 rounded-2xl border-2 flex items-center justify-between shadow-sm", sa.studioNet >= 0 ? "bg-white border-primary/5 hover:border-primary/20" : "bg-red-50 border-red-200")}>
                                    <div className="flex items-center gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0 text-left">
                                            ...
                                            <p className="text-[8px] font-bold text-muted-foreground uppercase opacity-60">{sa.payStructure === 'hourly_plus_commission' ? 'Hybrid' : sa.payStructure}</p>
                                        </div>
                                    </div>
  ...
  const { timeCost, productCosts, totalHardCost } = useMemo(() => {
    ...
    const productCosts = (service.products || []).map(p => {
        ...
        if (product) {
            ...
            cost = cpu * (p.quantityUsed || 1);
        }
        return { ...p, cost };
    });
    return { timeCost, productCosts, totalHardCost: timeCost + productCosts.reduce((a, p) => a + p.cost, 0) };
  }, [service, tmhr, inventory]);
    ...
    <Card className="border-2 shadow-sm rounded-[2rem] overflow-hidden bg-white">
        ...
        <CardContent className="p-6 sm:p-8 space-y-10 text-left">
            <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2 opacity-60"><Scale className="w-3.5 h-3.5" />Material Components</h4>
                <div className="flex justify-between items-center bg-muted/20 p-4 rounded-xl border-2">
                    <div className="text-left">
                        ...
                        <p className="text-[9px] font-bold text-muted-foreground uppercase">{service.products?.length || 0} Assets</p>
                    </div>
                    <span className="font-black font-mono text-sm">${productCosts.reduce((a, p) => a + p.cost, 0).toFixed(2)}</span>
                </div>
            </div>
            <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2 opacity-60"><Clock className="w-3.5 h-3.5" />Fixed Foundation Allocation</h4>
                <div className="flex justify-between items-center bg-muted/20 p-5 rounded-[1.5rem] border-2 gap-4">
                    <div className="space-y-0.5 text-left">
                        <p className="font-black text-sm uppercase">Reserved Studio Time</p>
...
export default function ServiceDetailPage() {
    ...
    const { services, appointments, staff, pricingTiers, resources, isLoading } = useInventory();
    ...
    const handleUpdateService = (updated: Service) => {
        ...
        toast({ title: "Service Updated", description: `${updated.name} has been saved.` });
    };

    const servicePerformance = useMemo(() => {
        if (!service) return null;
        const bookings = appointments.filter(apt => apt.serviceId === service.id && apt.status === 'completed');
        const totalRevenue = bookings.reduce((acc, apt) => acc + ((apt as any).revenue || service.price), 0);
        const uniqueClients = new Set(bookings.map(apt => apt.clientId)).size;
        return {
            totalBookings: bookings.length, totalRevenue, uniqueClients,
    ...
    const handleCopyLink = () => {
        ...
        navigator.clipboard.writeText(`${window.location.origin}/book/${selectedTenant.id}/${service.id}`);
        toast({ title: "Booking Link Copied!", description: "Direct URL ready for distribution." });
    };

    if (isLoading) return <div className="h-screen flex items-center justify-center"><Loader className="animate-spin text-primary h-10 w-10" /></div>;
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-slate-50/50 overflow-x-hidden">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-10 space-y-8 md:space-y-10 w-full max-w-7xl mx-auto min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                <Button variant="outline" size="sm" asChild className="flex-1 sm:flex-none h-12 px-4 md:px-6 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest bg-white/50 shadow-sm">
                    <Link href="/services" className="flex items-center"><ArrowLeft className="h-4 w-4 mr-2" />Return</Link>
                </Button>
                <Button variant="outline" size="sm" className="flex-1 sm:flex-none h-12 px-4 md:px-6 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest bg-white/50 shadow-sm" onClick={handleCopyLink}>
                    <LinkIcon className="h-4 w-4 mr-2"/>Share Link
                </Button>
                {/* FIXED: Modify button now opens the edit sheet */}
                <Button size="sm" className="flex-1 sm:flex-none h-12 px-4 md:px-6 rounded-2xl shadow-xl font-black uppercase text-[10px] tracking-widest shadow-primary/20" onClick={() => setIsEditOpen(true)}>
                    <Pencil className="h-4 w-4 mr-2" />Modify
                </Button>
            </div>
        </div>

        <Card className="border-4 shadow-3xl rounded-[2.5rem] md:rounded-[3rem] overflow-hidden bg-white/80 backdrop-blur-xl">
            <CardContent className="p-6 md:p-12 flex flex-col sm:flex-row items-center sm:items-start gap-6 md:gap-12">
                ...
                <div className="space-y-4 flex-1 min-w-0 text-left">
                    <div className="flex flex-wrap items-baseline gap-3">
                        <h2 className="text-2xl md:text-5xl font-black uppercase tracking-tighter text-slate-900 leading-none">{service.name}</h2>
                        {service.isPrivate && <Badge className="bg-muted text-muted-foreground border-none font-black text-[9px] uppercase tracking-widest h-6 px-3">Private</Badge>}
                        <Badge variant="outline" className="h-6 px-3 rounded-full font-black text-[9px] uppercase tracking-widest border-2">{service.category || 'Standard'}</Badge>
                    </div>
                    <div className="flex flex-wrap gap-x-10 gap-y-4 pt-2">
                        <div className="space-y-1">
                            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Duration</p>
                            <div className="flex items-center gap-2 font-black text-xl text-primary tracking-tight">
                                <Clock className="w-5 h-5" />{service.duration} MIN
                            </div>
                        </div>
                        {((service.padBefore || 0) + (service.padAfter || 0)) > 0 && (
                            <div className="space-y-1">
                                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Padding</p>
                                <p className="text-xl font-black uppercase tracking-tight text-slate-500">+{(service.padBefore || 0) + (service.padAfter || 0)}m</p>
                            </div>
                        )}
                        <div className="space-y-1">
                            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Base Rate</p>
                            <p className="text-xl font-black uppercase tracking-tight text-slate-700">${service.price.toFixed(2)}</p>
                        </div>
                    </div>
                    <p className="text-sm font-medium text-slate-600 leading-relaxed max-w-2xl pt-2">{service.description || 'No description provided.'}</p>
                </div>
            </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-10">
            <div className="lg:col-span-2 xl:col-span-3 space-y-10 min-w-0 order-2 lg:order-1">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        { label: 'Bookings', icon: CalendarIcon, value: servicePerformance?.totalBookings },
                        { label: 'Total Yield', icon: TrendingUp, value: `$${servicePerformance?.totalRevenue.toFixed(0)}` },
                        { label: 'Guests', icon: Users, value: servicePerformance?.uniqueClients },
                        { label: 'Avg. Value', icon: Target, value: `$${servicePerformance?.avgRevenuePerBooking.toFixed(0)}`, highlight: true },
                    ].map(({ label, icon: Icon, value, highlight }) => (
                        <Card key={label} className="border-2 shadow-sm rounded-3xl overflow-hidden bg-white">
                            ...
                            <CardContent className="p-4 pt-0 text-left">
                                <p className={cn("text-2xl md:text-3xl font-black tracking-tighter font-mono", highlight ? "text-primary" : "text-slate-900")}>{value}</p>
                            </CardContent>
                        </Card>
                    ))}
                </div>
                ...
                <Tabs defaultValue="architecture" className="w-full">
                    <TabsList className="bg-muted/30 p-1 rounded-2xl border-2 border-muted shadow-inner flex gap-1.5 mb-8 overflow-x-auto">
                        <TabsTrigger value="architecture" className="flex-1 min-w-[120px] h-11 rounded-xl font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-md">Architecture</TabsTrigger>
                        <TabsTrigger value="logistics" className="flex-1 min-w-[120px] h-11 rounded-xl font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-md">Logistics</TabsTrigger>
                    </TabsList>
                    <TabsContent value="architecture" className="m-0 animate-in fade-in duration-500">
                        <CostBreakdown service={service} tmhr={tmhr} />
                    </TabsContent>
                    <TabsContent value="logistics" className="m-0 animate-in fade-in duration-500">
                        <Card className="border-2 shadow-sm rounded-[2rem] overflow-hidden bg-white">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {(service.requiredResourceIds || []).map(rid => (
                                            <div key={rid} className="p-4 rounded-2xl bg-muted/20 border-2 flex items-center gap-4">
                                                ...
                                                <p className="font-black text-xs uppercase tracking-tight text-slate-900">Resource #{rid.slice(-4).toUpperCase()}</p>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
      ...
      {/* Edit sheet — wired to the Modify button */}
      <EditServiceDialog
        open={isEditOpen}
