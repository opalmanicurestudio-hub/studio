                        ...
                        <div className="p-5 rounded-[2rem] border-2 bg-green-50 border-green-200 space-y-5">
                          ...
                          <div className="space-y-3">
                            <div className="space-y-1.5">
                              ...
                              <Input value={geoStreet} onChange={e => setGeoStreet(e.target.value)} disabled={!isEditing} placeholder="123 Main Street, Suite 100" className="h-12 rounded-2xl border-2 font-bold bg-white border-green-200 focus:border-green-400" />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5">
                                <Label className="text-[9px] font-black uppercase tracking-widest text-green-800/60 ml-1">City</Label>
                                <Input value={geoCity} onChange={e => setGeoCity(e.target.value)} disabled={!isEditing} placeholder="Los Angeles" className="h-12 rounded-2xl border-2 font-bold bg-white border-green-200 focus:border-green-400" />
                              </div>
                              <div className="space-y-1.5">
                                <Label className="text-[9px] font-black uppercase tracking-widest text-green-800/60 ml-1">State</Label>
                                <Input value={geoState} onChange={e => setGeoState(e.target.value)} disabled={!isEditing} placeholder="CA" className="h-12 rounded-2xl border-2 font-bold bg-white border-green-200 focus:border-green-400" />
                              </div>
                            </div>
                            <div className="space-y-1.5">
                              <Label className="text-[9px] font-black uppercase tracking-widest text-green-800/60 ml-1">ZIP Code</Label>
                              <Input value={geoZip} onChange={e => setGeoZip(e.target.value)} disabled={!isEditing} placeholder="90001" className="h-12 rounded-2xl border-2 font-bold bg-white border-green-200 focus:border-green-400" />
                            </div>
                          </div>
                          ...
                          {tenantData.studioLocation && (
                            <div className="p-4 rounded-2xl bg-white border-2 border-green-200 space-y-2">
                              ...
                              <a href={`https://www.google.com/maps?q=${tenantData.studioLocation.lat},${tenantData.studioLocation.lng}`} target="_blank" rel="noopener noreferrer" className="ml-6 text-[9px] font-black uppercase text-green-600 underline underline-offset-2 hover:text-green-800">
                          ...
                          <Select value={tenantData.geoFenceFailBehavior || 'warn'} onValueChange={(v: any) => setTenantData(prev => ({ ...prev, geoFenceFailBehavior: v }))} disabled={!isEditing}>
                            ...
                            <SelectContent className="rounded-xl border-2 shadow-2xl">
                              <SelectItem value="warn" className="font-bold uppercase text-[10px] tracking-widest py-3">
                                <div className="space-y-0.5"><p>Warn Only</p><p className="text-[8px] opacity-40">Show warning but allow clock-in to proceed</p></div>
                              </SelectItem>
                              <SelectItem value="block" className="font-bold uppercase text-[10px] tracking-widest py-3">
                                <div className="space-y-0.5"><p>Hard Block</p><p className="text-[8px] opacity-40">Prevent clock-in entirely until inside zone</p></div>
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
