                ...
                {policy.mode !== 'inherit' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                        <CardContent className="p-4 space-y-6 text-left">
                            ...
                            {policy.mode !== 'matrix' && (
                                <div className="space-y-2 animate-in slide-in-from-top-2 text-left text-left">
...
function SettingsPageImpl() {
  ...
  const handleLoadStrategicTemplates = () => {
      setTenantData(prev => ({
          ...
          recoveryPresets: defaultRecoveryPresets,
          maxAutonomousRecoveryAmount: 50,
          maxAutonomousRecoveryPercent: 25,
      }));
      toast({ title: "Strategic Templates Loaded", description: "Best-practice recovery protocols and thresholds have been applied." });
  };
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto space-y-8 md:space-y-10 p-4 md:p-10 pb-32">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 text-left">
            <div className="space-y-1 text-left">
                <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tighter text-slate-900 leading-none text-left">Settings</h1>
                <p className="text-[10px] md:text-sm text-muted-foreground font-black uppercase tracking-[0.2em] opacity-60 text-left">Studio Orchestration & Governance</p>
            </div>
                    ...
                    <CardContent className="p-6 md:p-8 space-y-10 text-left">
                        <div className="space-y-4 max-w-sm text-left">
                            ...
                            <Select 
                                ...
                                disabled={!isEditing}
                            >
                                <SelectTrigger className="h-14 rounded-2xl border-2 font-black uppercase text-xs tracking-tight shadow-inner bg-muted/5 text-left">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="rounded-xl border-2 shadow-2xl">
            ...
            <TabsContent value="experience" className="mt-0 space-y-10 animate-in fade-in duration-500 text-left">
                <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden bg-white">
                    <CardHeader className="bg-muted/5 border-b p-6 md:p-8 text-left text-left">
                        ...
                        <CardDescription className="text-[10px] md:text-xs font-bold uppercase tracking-widest opacity-60 mt-1 text-left text-left">Configure the in-service refreshment and amenity module.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6 md:p-8 space-y-10 text-left text-left">
                        <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 bg-primary/5 shadow-inner border-primary/10 gap-6 text-left">
                            <div className="space-y-1 text-left text-left">
                                <Label htmlFor="refreshment-toggle" className="text-base font-black uppercase tracking-tight text-slate-900 text-left">Activate Refreshment Menu</Label>
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60 text-left text-left">Allows guests to request in-stock items from their portal</p>
                            </div>
                            <Switch 
                        ...
                        <div className="space-y-4 text-left">
                            <Label htmlFor="amenity-limit" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Complimentary Amenity Limit</Label>
                            <div className="space-y-3 text-left">
                                <Input 
                                    id="amenity-limit"
                                    type="number"
                                ...
                                />
                                <p className="text-[9px] font-bold text-muted-foreground uppercase leading-relaxed ml-1 opacity-60 text-left text-left">
                        ...
                        <div className="space-y-8 text-left">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1 text-left text-left">
                                <div className="space-y-1 text-left text-left">
                                    <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-900 flex items-center gap-2 text-left">
                                        <QrCode className="w-5 h-5 text-primary" />
                                        Station Identity Protocol
                                    </h3>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60 text-left">Generate physical place-cards for autonomous ordering.</p>
                                </div>
                                <Button onClick={() => setIsPrintStationsOpen(true)} className="h-12 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-primary/20 w-full md:w-auto">
                                    Generate Cards
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6 text-left">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 text-left text-left">
            ...
            <TabsContent value="policies" className="mt-0 space-y-10 animate-in fade-in duration-500 text-left">
                <div className="flex justify-end mb-4">
                    {isEditing && (
                        <Button 
                            variant="outline" 
                            size="sm" 
                            onClick={handleLoadStrategicTemplates}
                            className="h-9 px-4 rounded-xl border-2 border-primary/20 bg-primary/5 text-primary font-black uppercase text-[10px] tracking-widest shadow-sm hover:bg-primary/10 transition-all"
                        >
                            <Sparkles className="w-3.5 h-3.5 mr-2" />
                            Load Studio Best Practices
                        </Button>
                    )}
                </div>
                    ...
                    <CardContent className="p-6 md:p-8 space-y-10 text-left">
                        ...
                        <div className="space-y-8 text-left">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left text-left">
                                <div className="space-y-4 p-6 rounded-[2.5rem] border-2 bg-primary/5 border-primary/10 shadow-inner text-left">
                                    <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1 text-left">Autonomous Comp Limit</Label>
                                    <div className="flex gap-3 text-left">
                                        <div className="relative flex-1 text-left">
                                            ...
                                            <Input 
                                                ...
                                                disabled={!isEditing}
                                                className="h-14 pl-12 rounded-2xl border-2 font-black text-xl shadow-inner bg-white text-left"
                                            />
                                        </div>
                                        <div className="relative w-24 text-left">
                                ...
                                <div className="space-y-4 text-left text-left">
                                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Standing Escalation Orders</Label>
                                    <Textarea 
                            ...
                            <div className="space-y-6 pt-4 border-t border-dashed text-left text-left">
                                <div className="flex items-center justify-between px-1 text-left text-left">
                                    <div className="space-y-1 text-left text-left">
                                        ...
                                        <p className="text-[9px] font-bold text-muted-foreground uppercase opacity-60 text-left">One-tap recovery options for the POS terminal.</p>
                                    </div>
                                    {isEditing && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left text-left">
                                    {tenantData.recoveryPresets?.map(preset => (
                                        <div key={preset.id} className="p-4 rounded-2xl border-2 bg-white shadow-sm flex flex-col gap-4 group text-left">
                                            <div className="flex items-center justify-between gap-4 text-left">
                                                <div className="flex-1 text-left">
                                                    <Input 
                                                        ...
                                                        disabled={!isEditing}
                                                        className="h-9 border-none bg-transparent font-black uppercase tracking-tight text-xs p-0 focus-visible:ring-0 text-left"
                                                    />
                                                </div>
                                                {isEditing && (
                        ...
                        <div className="space-y-6 text-left">
                            <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 border-primary/20 bg-primary/5 shadow-xl shadow-primary/5 transition-all text-left">
                                <div className='space-y-1 text-left text-left'>
                                    <Label htmlFor="guardian-protocol-toggle-edit" className="text-base font-black uppercase tracking-tight text-primary flex items-center gap-2 text-left">
                                        <ShieldCheck className="w-4 h-4" /> Guardian Revenue Shield
                                    </Label>
                                    <p className='text-[10px] font-bold text-primary/60 uppercase tracking-widest opacity-60 text-left text-left'>Forced deposit enforcement for high-risk behavioral profiles</p>
                                </div>
                                <Switch 
                                    id="guardian-protocol-toggle-edit" 
                            ...
                            <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 border-indigo-500/20 bg-indigo-500/5 shadow-xl shadow-indigo-500/5 transition-all text-left">
                                <div className='space-y-1 text-left text-left'>
                                    <Label htmlFor="morning-anchor-toggle-edit" className="text-base font-black uppercase tracking-tight text-indigo-700 flex items-center gap-2 text-left">
                                        <Clock className="w-4 h-4" /> Morning Anchor Protocol
                                    </Label>
                                    <p className='text-[10px] font-bold text-indigo-600/60 uppercase tracking-widest opacity-60 text-left text-left'>The first appointment of an empty day must start at business opening time</p>
                                </div>
                                <Switch 
                                    id="morning-anchor-toggle-edit" 
                            ...
                            <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 border-primary/20 bg-primary/5 shadow-xl shadow-primary/5 transition-all text-left">
                                <div className='space-y-1 text-left text-left'>
                                    <Label htmlFor="tight-scheduling-toggle-edit" className="text-base font-black uppercase tracking-tight text-primary flex items-center gap-2 text-left text-left">
                                        <Workflow className="w-4 h-4" /> Zero-Gap Adjacency Protocol
                                    </Label>
                                    <p className='text-[10px] font-bold text-primary/60 uppercase tracking-widest opacity-60 text-left text-left'>Force client bookings to be flush against existing blocks</p>
                                </div>
                                <Switch 
                                    id="tight-scheduling-toggle-edit" 
                            ...
                            <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 border-amber-500/20 bg-amber-500/5 shadow-xl shadow-amber-500/5 transition-all text-left">
                                <div className='space-y-1 text-left text-left'>
                                    <Label htmlFor="flash-yield-toggle-edit" className="text-base font-black uppercase tracking-tight text-amber-700 flex items-center gap-2 text-left text-left">
                                        <Flame className="w-4 h-4" /> Flash Yield Protocol
                                    </Label>
                                    <p className='text-[10px] font-bold text-amber-600/60 uppercase tracking-widest opacity-60 text-left text-left'>Flag 48h cancellations as "magnetic" slots that bypass standard restrictions</p>
                                </div>
                                <Switch 
                                    id="flash-yield-toggle-edit" 
                        ...
                        <div className="space-y-6 text-left text-left">
                            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary ml-1 text-left text-left">Default Recovery Strategy</Label>
                            <Controller
                                name="defaultCancellationMode"
                                control={control}
                                defaultValue={tenantData.defaultCancellationMode || 'matrix'}
                                render={({ field }) => (
                                    <RadioGroup onValueChange={(v: any) => setTenantData(p => ({...p, defaultCancellationMode: v}))} value={tenantData.defaultCancellationMode || 'matrix'} className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
                                        <label htmlFor="mode-matrix-edit" className="cursor-pointer h-full">
                                            <div className={cn(
                                                ...
                                                <RadioGroupItem value="matrix" id="mode-matrix-edit" className="sr-only" disabled={!isEditing} />
                                            </div>
                                        </label>
                                        <label htmlFor="mode-percent-edit" className="cursor-pointer h-full">
                                            <div className={cn(
                                                "flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-all h-full text-center",
                                                ...
                                                <RadioGroupItem value="percentage" id="mode-percent-edit" className="sr-only" disabled={!isEditing} />
                                            </div>
                                        </label>
                                        <label htmlFor="mode-flat-edit" className="cursor-pointer h-full">
                                            <div className={cn(
                                                "flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-all h-full text-center",
                                                ...
                                                <RadioGroupItem value="flat" id="mode-flat-edit" className="sr-only" disabled={!isEditing} />
                                            </div>
                                        </label>
                                    </RadioGroup>
                                )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-dashed text-left">
                            <div className="space-y-3 text-left">
                                <Label htmlFor="cancel-window-edit-manual" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Cancellation Window (Hours)</Label>
                                <Input 
                                    id="cancel-window-edit-manual"
                                    type="number"
                            ...
                            <div className="space-y-3 text-left">
                                <Label htmlFor="cancel-fee-edit-manual" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Fixed Flat Fee ($)</Label>
                                <div className="relative text-left">
                                    <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-primary opacity-40" />
                                    <Input 
                                        id="cancel-fee-edit-manual"
                                        type="number"
                        ...
                        <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 border-dashed bg-muted/5 shadow-inner">
                            <div className="space-y-1 text-left text-left">
                                <Label htmlFor="allow-deferral-edit" className="text-base font-black uppercase tracking-tight flex items-center gap-2 text-left">
                                    <Zap className="w-4 h-4 text-primary" /> Guest Autonomy: Fee Deferral
                                </Label>
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60 text-left text-left">Allow guests to add rescheduling fees to their session bill</p>
                            </div>
                            <Switch 
                                id="allow-deferral-edit" 
                ...
                <div className="space-y-8 text-left">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-1 text-left">
                        <div className="space-y-1 text-left text-left">
                            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-slate-900 leading-none text-left">Service-Specific Protocols</h2>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60 text-left">Custom logic guards per treatment unit.</p>
                        </div>
                ...
                <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden bg-white">
                    <CardHeader className="bg-muted/5 border-b p-6 md:p-8 text-left text-left text-left">
                        <SectionHeader icon={Shield} title="Public Accountability Policies" />
                        <CardDescription className="text-[10px] md:text-xs font-bold uppercase tracking-widest opacity-60 mt-1 text-left text-left">Define the narrative for your guest-facing agreements.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6 md:p-8 space-y-6 text-left text-left text-left">
                        {[
                            ...
                            <div key={policy.id} className="space-y-2 text-left">
                                <Label htmlFor={`${policy.id}-input`} className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{policy.label}</Label>
                                <Textarea 
                                    id={`${policy.id}-input`}
            ...
            <TabsContent value="builder" className="mt-0 space-y-10 animate-in fade-in duration-500 text-left">
                <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden bg-white">
                    <CardHeader className="bg-muted/5 border-b p-6 md:p-8 text-left text-left">
                        ...
                        <CardDescription className="text-[10px] md:text-xs font-bold uppercase tracking-widest opacity-60 mt-1 text-left text-left">Configure your guest-facing digital presence.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6 md:p-8 space-y-10 text-left text-left">
                        <div className="space-y-8 text-left">
                            <div className="space-y-2 text-left text-left">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Brand Signature (Logo)</Label>
                                <ImageUpload 
                        ...
                        <div className="space-y-4 text-left">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Visibility Protocol</Label>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-left">
                                {[
                                    ...
                                    <div key={toggle.key} className="flex items-center justify-between p-4 rounded-2xl border-2 bg-muted/5 shadow-inner text-left">
                                        <span className="text-xs font-black uppercase tracking-tight text-left">{toggle.label}</span>
                    ...
                    <CardContent className="p-6 md:p-8 space-y-10 text-left text-left">
                        <div className="space-y-8 text-left text-left">
                            ...
                            <div className="flex items-center justify-between p-6 rounded-[2rem] border-2 bg-primary/5 shadow-inner border-primary/10 transition-all text-left">
                                <div className="space-y-1 text-left">
                                    <Label htmlFor="kiosk-hours-toggle-edit" className="text-base font-black uppercase tracking-tight text-primary">Specific Kiosk Hours</Label>
