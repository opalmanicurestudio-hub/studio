...
function SettingsPageImpl() {
  ...
  const tabs = [
    ...
    { value: "experience", label: "Guest Experience", icon: <Coffee className="w-4 h-4" /> },
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-slate-50/50">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto space-y-10 p-4 md:p-10 pb-32">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 text-left">
            <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground font-black uppercase tracking-[0.2em] opacity-60">Studio Orchestration & Governance</p>
            </div>
            <div className="flex items-center gap-3">
                {isEditing ? (
                    <>
                        ...
                        <Button onClick={handleSave} className="h-12 px-8 rounded-2xl shadow-xl font-black uppercase text-[10px] tracking-widest shadow-primary/20"><Save className="mr-2 h-4 w-4" />Save Changes</Button>
                    </>
                ) : (
                    <Button onClick={() => setIsEditing(true)} className="h-12 px-8 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest bg-white shadow-sm"><Edit className="mr-2 h-4 w-4" />Modify Logic</Button>
                )}
            </div>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
             <ScrollArea className="w-full">
                <TabsList className="bg-muted/30 p-1 rounded-2xl border-2 shadow-inner mb-8 flex w-max">
                    {tabs.map(tab => (
            ...
            <TabsContent value="experience" className="mt-0 space-y-10 animate-in fade-in duration-500">
                <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden">
                    <CardHeader className="bg-muted/5 border-b p-8 text-left">
                        ...
                        <CardTitle className="text-2xl font-black uppercase tracking-tighter">Hospitality Concierge</CardTitle>
                        <CardDescription className="text-xs font-bold uppercase tracking-widest opacity-60">Configure the in-service refreshment and amenity module.</CardDescription>
                    </CardHeader>
            ...
            <TabsContent value="profile" className="mt-0 text-left">
                <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden bg-white">
                    <CardHeader className="bg-muted/5 border-b p-8 text-left">
                        <CardTitle className="flex items-center gap-3 text-base font-black uppercase tracking-tight">
                    ...
                    <CardContent className="p-8 space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input 
                                ...
                                disabled={!isEditing}
                                className="h-14 rounded-2xl border-2 font-black uppercase text-xl tracking-tighter"
                            />
                        </div>
                    </CardContent>
                </Card>
...
export default function SettingsPage() {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader className="animate-spin h-8 w-8 text-primary" /></div>}>
      <SettingsPageImpl />
    </Suspense>
  )
}
