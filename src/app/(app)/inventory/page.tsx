    ...
    <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden bg-white cursor-pointer hover:shadow-md hover:border-primary/20 transition-all group" onClick={() => onSelect(order)}>
      <CardContent className="p-6 space-y-4">
        ...
        <div className="space-y-1 text-left">
          {order.items.slice(0, 2).map((item, i) => (<p key={i} className="text-[10px] font-bold text-muted-foreground truncate">· {item.productName} ×{item.quantity}</p>))}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-dashed">
          ...
          <div className="flex gap-2">
            {order.trackingNumber && (<Button variant="outline" size="sm" className="h-9 rounded-xl font-black uppercase text-[9px] tracking-widest border-2" onClick={(e) => { e.stopPropagation(); onTrack(e, order.trackingUrl); }}><Truck className="mr-1.5 h-3 w-3" /> Track</Button>)}
            {canReceive && (<Button size="sm" className="h-9 rounded-xl font-black uppercase text-[9px] tracking-widest shadow-md shadow-primary/20" onClick={(e) => { e.stopPropagation(); onReceive(order); }}><PackageOpen className="mr-1.5 h-3 w-3" /> Receive</Button>)}
          </div>
        </div>
      </CardContent>
    </Card>
...
const ViewOrEditOrderDialog = ({ order, open, onOpenChange, onSave, onCancelOrder, onTrack }: { order: Order | null; open: boolean; onOpenChange: (open: boolean) => void; onSave: (order: Order) => void; onCancelOrder: (orderId: string) => void; onTrack: (e: React.MouseEvent, url?: string) => void; }) => {
  ...
  useEffect(() => { if (order) { setNotes(order.notes || ''); setTrackingNumber(order.trackingNumber || ''); setTrackingUrl(order.trackingUrl || ''); setExpectedDate(order.expectedArrivalDate || ''); setIsEditing(false); } }, [order]);
      ...
      <DialogContent className="sm:max-w-2xl p-0 overflow-hidden border-4 rounded-[3rem] shadow-3xl max-h-[90dvh] flex flex-col">
        <DialogHeader className="p-8 pb-0 text-left flex-shrink-0">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1"><p className="text-[10px] font-black uppercase tracking-widest text-primary/60">Purchase Order</p><DialogTitle className="text-2xl font-black uppercase tracking-tighter">{order.supplier}</DialogTitle><DialogDescription className="text-[10px] font-bold uppercase tracking-widest opacity-60">#{order.id.slice(-6).toUpperCase()} · Placed {format(parseISO(order.orderDate), 'MMM d, yyyy')}</DialogDescription></div>
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="p-8 space-y-6">
            <div className="space-y-2"><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Order Items</p><div className="rounded-2xl border-2 overflow-hidden"><Table><TableHeader className="bg-muted/10"><TableRow><TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-900 p-4">Product</TableHead><TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-900 text-right">Qty</TableHead><TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-900 text-right pr-4">Cost</TableHead></TableRow></TableHeader><TableBody>{order.items.map((item, i) => (<TableRow key={i}><TableCell className="p-4 font-bold text-sm text-slate-900">{item.productName}</TableCell><TableCell className="text-right font-black font-mono">{item.quantity}</TableCell><TableCell className="text-right pr-4 font-black font-mono text-primary">${(item.quantity * item.costPerUnit).toFixed(2)}</TableCell></TableRow>))}<TableRow className="bg-muted/10"><TableCell colSpan={2} className="p-4 font-black uppercase text-[10px] tracking-widest text-right">Total</TableCell><TableCell className="text-right pr-4 font-black font-mono text-lg text-primary">${totalCost.toFixed(2)}</TableCell></TableRow></TableBody></Table></div></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Tracking Number</Label>{isEditing ? (<Input value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} className="h-12 rounded-xl border-2 font-mono font-black text-xs" placeholder="e.g., 1Z999AA10123456784" />) : (<div className="h-12 rounded-xl border-2 border-dashed flex items-center px-4 gap-2"><span className="font-mono font-black text-xs text-slate-600">{trackingNumber || '—'}</span>{trackingNumber && (<Button variant="ghost" size="icon" className="h-7 w-7 ml-auto" onClick={(e) => onTrack(e, trackingUrl || undefined)}><ExternalLink className="h-3.5 w-3.5" /></Button>)}</div>)}</div>
              <div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Expected Arrival</Label>{isEditing ? (<Input type="date" value={expectedDate ? expectedDate.split('T')[0] : ''} onChange={(e) => setExpectedDate(e.target.value ? new Date(e.target.value).toISOString() : '')} className="h-12 rounded-xl border-2 font-black text-xs" />) : (<div className="h-12 rounded-xl border-2 border-dashed flex items-center px-4"><span className="font-black text-xs text-slate-600">{expectedDate ? format(parseISO(expectedDate), 'MMM d, yyyy') : '—'}</span></div>)}</div>
            </div>
            {isEditing && (<div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Tracking URL</Label><Input value={trackingUrl} onChange={(e) => setTrackingUrl(e.target.value)} className="h-12 rounded-xl border-2 font-black text-xs" placeholder="https://track.carrier.com/..." /></div>)}
            <div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Notes</Label>{isEditing ? (<Textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="rounded-2xl border-2 bg-muted/5 min-h-[80px]" placeholder="Internal notes about this order..." />) : (<div className="rounded-2xl border-2 border-dashed p-4 min-h-[60px]"><p className="text-sm font-medium text-slate-600 whitespace-pre-wrap">{notes || <span className="opacity-30 font-black text-[10px] uppercase">No notes</span>}</p></div>)}</div>
          </div>
        </ScrollArea>
        <DialogFooter className="p-8 pt-0 border-t bg-muted/5 flex-shrink-0"><div className="flex flex-col gap-3 w-full">{isEditing ? (<div className="flex gap-3"><Button variant="outline" className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest border-2" onClick={() => setIsEditing(false)}>Cancel</Button><Button className="flex-[2] h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-primary/20" onClick={handleSave}><Save className="mr-2 h-4 w-4" /> Save Changes</Button></div>) : (<div className="flex gap-3"><Button variant="outline" className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest border-2" onClick={() => setIsEditing(true)}><Edit className="mr-2 h-4 w-4" /> Edit Order</Button>{canCancel && (<Button variant="destructive" className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest" onClick={() => { onOpenChange(false); onCancelOrder(order.id); }}><XCircle className="mr-2 h-4 w-4" /> Cancel Order</Button>)}</div>)}<Button variant="ghost" className="h-10 font-bold uppercase text-[10px] tracking-widest text-muted-foreground" onClick={() => onOpenChange(false)}>Close</Button></div></DialogFooter>
      </DialogContent>
    </Dialog>
  );
};
...
const OrdersTab = ({ inventory }: { inventory: InventoryItem[] }) => {
    ...
    const handleAddOrder = (newOrderData: Omit<Order, 'id'>) => {
        ...
        newOrderData.items.forEach(item => {
            if (item.productId.startsWith('custom-')) {
                ...
                const newProductShell: InventoryItem = { id: newProductId, name: item.productName, type: 'professional', category: 'Uncategorized', totalStock: 0, supplier: newOrderData.supplier, costPerUnit: item.costPerUnit, batches: [] };
                setDocumentNonBlocking(doc(firestore, `tenants/${tenantId}/inventory`, newProductId), newProductShell, {});
        ...
        if (totalCost > 0) {
            const newTransaction: Omit<Transaction, 'id' | 'date'> = { description: `Purchase Order: ${newOrder.supplier}`, clientOrVendor: newOrder.supplier, type: 'expense', context: newOrder.paymentContext || 'Business', category: 'Supplies', amount: totalCost, paymentMethod: newOrder.paymentMethod || 'On Account', paymentMethodIdentifier: newOrder.paymentMethodIdentifier, hasReceipt: !!newOrder.invoiceUrl, receiptUrl: newOrder.invoiceUrl, relatedOrderId: orderId };
            addDocumentNonBlocking(collection(firestore, 'tenants', tenantId, 'transactions'), { ...newTransaction, date: newOrder.orderDate });
        }
    ...
    const handleConfirmCancelOrder = () => {
        ...
        if (totalCost > 0) {
            const newTransaction: Omit<Transaction, 'id' | 'date'> = { description: `Reversal for Order: ${orderToCancel.supplier}`, clientOrVendor: orderToCancel.supplier, type: 'reversal', context: orderToCancel.paymentContext || 'Business', category: 'Supplies', amount: totalCost, paymentMethod: 'On Account', hasReceipt: !!orderToCancel.invoiceUrl, receiptUrl: orderToCancel.invoiceUrl, relatedOrderId: orderToCancel.id };
    ...
    const filteredOrders = useMemo(() => {
        ...
        return orders.filter(order => {
            ...
            const searchMatch = searchTerm === '' || order.supplier.toLowerCase().includes(sl) || order.id.toLowerCase().includes(sl) || order.items.some(item => item.productName.toLowerCase().includes(sl));
    ...
    const handleReceiveStock = (receivedItems: ReceivedItem[]) => {
        ...
        receivedItems.forEach(item => {
            ...
            if (existingProduct) {
                ...
                if (item.quantityReceived > 0) {
                    const newBatchData: any = { id: `batch-${nanoid()}`, stock: item.quantityReceived, costPerUnit: item.costPerUnit, receivedDate: new Date().toISOString() };
                    if (item.expirationDate) newBatchData.expirationDate = item.expirationDate.toISOString();
                    const updatedBatches = [...existingProduct.batches, newBatchData];
                    const totalStock = updatedBatches.reduce((acc, b) => acc + b.stock, 0);
                    batch.update(productRef, JSON.parse(JSON.stringify({ batches: updatedBatches, totalStock, costPerUnit: item.costPerUnit })));
                    const scRef = doc(collection(firestore, `tenants/${tenantId}/stockCorrections`));
                    batch.set(scRef, { productId: item.productId, date: new Date().toISOString(), change: item.quantityReceived, unit: existingProduct.unit || 'units', reason: `Shipment from ${orderToReceive.supplier}` });
                }
                if (item.quantityDamaged > 0) {
                    const dtRef = doc(collection(firestore, `tenants/${tenantId}/transactions`));
                    batch.set(dtRef, { date: new Date().toISOString(), description: `Damaged on arrival: ${item.quantityDamaged} x ${item.productName}`, clientOrVendor: orderToReceive.supplier, type: 'expense', context: 'Business', category: 'Spoilage', amount: item.quantityDamaged * item.costPerUnit, paymentMethod: 'Internal', hasReceipt: !!orderToReceive.invoiceUrl, receiptUrl: orderToReceive.invoiceUrl, relatedOrderId: orderToReceive.id });
                }
            }
        });
        const allFullyReceived = receivedItems.every(item => item.quantityReceived + item.quantityDamaged >= item.quantityOrdered);
        const someReceived = receivedItems.some(item => item.quantityReceived > 0 || item.quantityDamaged > 0);
        ...
        batch.commit().then(() => { toast({ title: "Stock Updated!", description: "Inventory has been updated with the received items." }); setOrderToReceive(null); }).catch(error => { console.error(error); toast({ variant: "destructive", title: "Error", description: "Failed to update stock." }); });
    };
        ...
        <div className="space-y-8 text-left">
            <div className="space-y-6">
                ...
                <div className="p-4 md:p-6 bg-primary/[0.03] rounded-3xl border-2 border-dashed border-primary/20 flex flex-col md:flex-row items-center gap-4">
                    ...
                    <div className="w-full md:w-auto"><Select value={statusFilter} onValueChange={(value) => setStatusFilter(value)}><SelectTrigger className="h-14 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest w-full md:w-48 bg-white shadow-inner"><SelectValue placeholder="STATUS" /></SelectTrigger><SelectContent className="rounded-xl border-2 shadow-2xl"><SelectItem value="all" className="font-bold">ALL STATUSES</SelectItem><SelectItem value="Draft" className="font-bold">DRAFT</SelectItem><SelectItem value="Placed" className="font-bold">PLACED</SelectItem><SelectItem value="Shipped" className="font-bold">SHIPPED</SelectItem><SelectItem value="Partially Received" className="font-bold text-amber-600">PARTIAL</SelectItem><SelectItem value="Received" className="font-bold text-green-600">RECEIVED</SelectItem><SelectItem value="Cancelled" className="font-bold text-destructive">CANCELLED</SelectItem></SelectContent></Select></div>
                </div>
            </div>
            ...
            <AlertDialog open={!!orderToCancel} onOpenChange={() => setOrderToCancel(null)}>
                <AlertDialogContent className="rounded-[3rem] border-4 shadow-3xl">
                    ...
                    <div className="p-6 space-y-3 text-left"><Label htmlFor="cancel-reason" className="text-[10px] font-black uppercase tracking-widest ml-1 opacity-60">Audit Note</Label><Textarea id="cancel-reason" placeholder="Reason for protocol termination..." value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} className="rounded-2xl border-2 bg-muted/5" /></div>
...
const EmptyOrdersState = ({ onAddFirstOrder }: { onAddFirstOrder: () => void }) => (
    <div className="text-center py-24 px-6 col-span-full border-4 border-dashed rounded-[3rem] opacity-40 flex flex-col items-center gap-6">
        ...
        <div className="space-y-2"><h3 className="text-2xl font-black uppercase tracking-tighter text-slate-900">Procurement Clear</h3><p className="text-sm font-bold uppercase tracking-tight text-muted-foreground mx-auto">No supply orders in the ledger.</p></div>
...
export default function InventoryPage() {
  ...
  useEffect(() => { if (inventory) { const allCategories = inventory.map(p => p.category).filter((c): c is string => !!c); setProductCategories([...new Set(allCategories)]); } }, [inventory]);
  const onNewCategory = useCallback((newCategory: string) => { if (!productCategories.includes(newCategory)) setProductCategories(prev => [...prev, newCategory].sort()); }, [productCategories]);

  const ordersQuery = useMemoFirebase(() => tenantId ? collection(firestore, `tenants/${tenantId}/orders`) : null, [firestore, tenantId]);
  const { data: orders } = useCollection<Order>(ordersQuery);
  const orderedProductIds = useMemo(() => { if (!orders) return new Set(); const activeOrders = orders.filter(o => o.status === 'Placed' || o.status === 'Shipped'); const productIds = new Set<string>(); activeOrders.forEach(order => order.items.forEach(item => productIds.add(item.productId))); return productIds; }, [orders]);
  ...
  const handleItemSelect = useCallback((itemId: string) => { setSelectedItems(prev => { const newSelection = new Set(prev); if (newSelection.has(itemId)) newSelection.delete(itemId); else newSelection.add(itemId); return newSelection; }); }, []);
  const handleBulkDeleteClick = () => setIsBulkDeleteConfirmOpen(true);
  const handleBulkDeleteConfirm = useCallback(() => { if (!firestore || !tenantId) return; const itemCount = selectedItems.size; const batch = writeBatch(firestore); selectedItems.forEach(id => batch.delete(doc(firestore, `tenants/${tenantId}/inventory`, id))); batch.commit(); setSelectedItems(new Set()); setIsBulkDeleteConfirmOpen(false); toast({ title: "Items Deleted", description: `${itemCount} item(s) have been removed from your inventory.` }); }, [selectedItems, toast, firestore, tenantId]);
  ...
  const handleProductAdded = (newProduct: InventoryItem) => { if (!firestore || !tenantId) return; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'inventory', newProduct.id), JSON.parse(JSON.stringify(newProduct)), {}); toast({ title: `New ${newProduct.type} product created`, description: `${newProduct.name} has been added to your inventory.` }); };
  const handleEquipmentAdded = (newEquipment: InventoryItem) => { if (!firestore || !tenantId) return; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'inventory', newEquipment.id), JSON.parse(JSON.stringify(newEquipment)), {}); };
  const handleOverheadAdded = (newOverhead: InventoryItem) => { if (!firestore || !tenantId) return; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'inventory', newOverhead.id), JSON.parse(JSON.stringify(newOverhead)), {}); };
  const handleRefreshmentAdded = (newItem: InventoryItem) => { if (!firestore || !tenantId) return; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'inventory', newItem.id), JSON.parse(JSON.stringify(newItem)), {}); toast({ title: "Amenity Registered", description: `${newItem.name} is now available in your hospitality manifest.` }); };
  ...
  const handleSaveLocation = (newLocation: Omit<Location, 'id'>) => { if (!firestore || !tenantId) return {} as Location; const newLocWithId = { ...newLocation, id: `loc-${nanoid()}` }; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'locations', newLocWithId.id), JSON.parse(JSON.stringify(newLocWithId)), {}); return newLocWithId; };
  ...
  const handleAddNewLocationType = (name: string, icon: string): LocationType => { if (!firestore || !tenantId) return { id: '', name: '', icon: '' }; const newType = { id: `lt-${nanoid()}`, name, icon }; setDocumentNonBlocking(doc(firestore, 'tenants', tenantId, 'locationTypes', newType.id), JSON.parse(JSON.stringify(newType)), {}); return newType; };
  ...
  const handleWriteOffConfirm = useCallback((productId: string, batchId: string, quantity: number, reason: string, notes?: string, imageUrl?: string): { success: boolean, message: string } => {
    ...
    if (batchToUpdate.stock < quantity) return { success: false, message: `Cannot write off more than available stock (${batchToUpdate.stock}).` };
  ...
  const handleLogSaleConfirm = (productId: string, quantity: number, paymentMethod: string): { success: boolean; message: string; } => {
    ...
    for (const batch of sortedBatches) { if (remainingToDeduct <= 0) break; const deductFromBatch = Math.min(batch.stock, remainingToDeduct); batch.stock -= deductFromBatch; remainingToDeduct -= deductFromBatch; }
    const newTotalStock = sortedBatches.reduce((acc, b) => acc + b.stock, 0);
    updateDocumentNonBlocking(doc(firestore, `tenants/${tenantId}/inventory`, productId), { totalStock: newTotalStock, batches: sortedBatches });
  ...
  const handleSpoilageConfirm = (items: SpoilageItem[], notes?: string, imageUrl?: string) => {
    ...
    items.forEach(item => {
      ...
      if (product) {
        const updatedBatches = product.batches.map(b => { if (b.id === item.batchId) { totalLoss += item.stock * item.costPerUnit; return { ...b, stock: 0 }; } return b; });
        const newTotalStock = updatedBatches.reduce((acc, b) => acc + b.stock, 0);
        const updatePayload: Partial<InventoryItem> = { batches: updatedBatches, totalStock: newTotalStock };
        if (newTotalStock === 0) { updatePayload.partialContainerUses = 0; updatePayload.partialContainerSize = 0; }
        batch.update(doc(firestore, `tenants/${tenantId}/inventory`, item.productId), JSON.parse(JSON.stringify(updatePayload)));
    ...
    batch.commit().then(() => { toast({ title: "Spoilage Written Off", description: `${items.length} item(s) written off. Total loss: $${totalLoss.toFixed(2)}.` }); }).catch(error => { console.error(error); toast({ variant: "destructive", title: "Error", description: "Failed to write off spoilage." }); });
  };
  ...
  const paginatedItems = useMemo(() => { const start = (currentPage - 1) * ITEMS_PER_PAGE; return filteredInventory.slice(start, start + ITEMS_PER_PAGE); }, [filteredInventory, currentPage]);
  const handlePrevPage = () => setCurrentPage(prev => Math.max(prev - 1, 1));
  const handleNextPage = () => setCurrentPage(prev => Math.min(prev + 1, totalPages));

  const handleScan = useCallback((data: string) => {
    const rawData = data.trim();
    if (rawData.startsWith('clarityflow://product/')) { const productId = rawData.split('/').pop(); if (productId) { setSearchTerm(productId); toast({ title: "Product Found", description: `Displaying results for scanned product.` }); } } else { setSearchTerm(rawData); toast({ title: "Scanning...", description: `Searching for code: ${rawData}` }); }
  }, [toast]);
      ...
      const timer = setTimeout(() => {
        ...
        if (element) {
          ...
          html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } },
            (decodedText: string) => { if (html5QrCode?.isScanning) html5QrCode.stop().catch(console.error); handleScan(decodedText); setIsScannerOpen(false); },
            () => {}
          ).catch(() => { toast({ variant: 'destructive', title: 'Camera Error', description: 'Could not start the camera.' }); setIsScannerOpen(false); });
        }
      }, 300);
      return () => { clearTimeout(timer); if (html5QrCode && html5QrCode.isScanning) html5QrCode.stop().catch(console.error); };
    }
      ...
      <main className="flex-1 p-4 md:p-10 w-full max-w-7xl mx-auto min-w-0">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10 text-left">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            {/* New Entry dropdown removed — replaced by FloatingInventoryFAB below */}
          </div>
        </div>
          ...
          <div className="lg:col-span-2 xl:col-span-3 space-y-8 min-w-0">
            ...
            <Card className="border-2 shadow-sm rounded-[2.5rem] overflow-hidden">
              <CardHeader className="bg-muted/5 border-b p-6 md:p-8 space-y-8 text-left">
                ...
                <div className="p-4 md:p-6 bg-primary/[0.03] rounded-3xl border-2 border-dashed border-primary/20 flex flex-wrap items-center gap-x-6 gap-y-4">
                  ...
                  <TabsContent value="products" className="mt-0">
                    {!hasInventory && !isInventoryLoading ? (<EmptyState onAddFirstItem={() => handleOpenAddProductDialog('professional')} />) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {hasFilteredInventory ? paginatedItems.map(item => (<ProductCard key={item.id} item={item} onEdit={handleEditItem} onToggleExperiment={handleToggleExperiment} onEndExperiment={handleEndExperiment} onLogUse={handleOpenLogUse} onWriteOff={handleOpenWriteOff} onLogSale={handleOpenLogSale} isSelected={selectedItems.has(item.id)} onSelect={() => handleItemSelect(item.id)} isOrdered={orderedProductIds.has(item.id)} />)) : (<div className="text-center py-24 opacity-30 border-4 border-dashed rounded-[3rem] flex flex-col items-center gap-4 col-span-full"><Filter className="w-16 h-16" /><p className="font-black uppercase tracking-widest text-sm">No Assets Found</p></div>)}
                      </div>
                    )}
                  </TabsContent>
        ...
        <Dialog open={isScannerOpen} onOpenChange={setIsScannerOpen}>
          <DialogContent className="sm:max-w-md p-0 overflow-hidden border-4 rounded-[3rem] shadow-3xl">
            ...
            <div className="p-8 relative"><div id="qr-reader-inventory" className="w-full aspect-square rounded-3xl bg-muted shadow-inner" /><div className="absolute inset-8 flex items-center justify-center pointer-events-none"><div className="w-2/3 h-2/3 border-4 border-primary rounded-3xl shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]" /></div></div>
        ...
        <AlertDialog open={isBulkDeleteConfirmOpen} onOpenChange={setIsBulkDeleteConfirmOpen}>
          <AlertDialogContent className="rounded-[3rem] border-4 shadow-3xl">
            <AlertDialogHeader className="p-6 pb-0"><AlertDialogTitle className="text-2xl font-black uppercase tracking-tighter">Terminate Assets</AlertDialogTitle><AlertDialogDescription className="font-bold text-sm text-slate-600 leading-relaxed uppercase">You are about to permanently delete {selectedItems.size} items. <strong>This action is non-reversible.</strong></AlertDialogDescription></AlertDialogHeader>
        ...
        {/* ── Floating action button — replaces the header dropdown ── */}
