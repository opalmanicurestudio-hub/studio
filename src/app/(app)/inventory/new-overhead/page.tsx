...
export default function NewOverheadPage() {
  ...
  const { onOverheadAdded, overheadCategories, onNewCategory, locations } = useInventory();
  const [isAddingCategory, setIsAddingCategory] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState('');

  const methods = useForm<OverheadFormData>({
    resolver: zodResolver(overheadSchema),
    defaultValues: { costingMethod: 'uses' }
  });
  ...
  const handleSave = (data: OverheadFormData) => {
    ...
    toast({
        title: "Overhead Item Added!",
        description: `${newOverhead.name} has been added to your inventory.`
    });
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-muted/40">
      <AppHeader title="Add Overhead Supply" />
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(handleSave)}>
                <Card>
                    <CardHeader>
                        <CardTitle>Add Overhead Supply</CardTitle>
                        <CardDescription>Log consumable supplies not directly tied to services.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <Controller name="name" control={control} render={({ field }) => ( <div className="space-y-2"><Label htmlFor="item-name">Item Name</Label><Input id="item-name" placeholder="e.g., Disinfectant Wipes" {...field} />{methods.formState.errors.name && <p className="text-sm text-destructive">{methods.formState.errors.name.message}</p>}</div> )}/>
                        <Controller name="category" control={control} render={({ field }) => ( <div className="space-y-2"> <Label htmlFor="category">Category</Label> {isAddingCategory ? ( <div className="flex gap-2"><Input placeholder="New category..." value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} /><Button onClick={handleAddNewCategory} type="button">Add</Button></div> ) : ( <div className="flex gap-2"> <Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger><SelectContent>{overheadCategories.map(cat => (<SelectItem key={cat} value={cat}>{cat}</SelectItem>))}</SelectContent></Select> <Button variant="outline" size="icon" onClick={() => setIsAddingCategory(true)} type="button"><PlusCircle className="h-4 w-4" /></Button> </div> )} {methods.formState.errors.category && <p className="text-sm text-destructive">{methods.formState.errors.category.message}</p>} </div> )}/>
                        <div className="grid grid-cols-2 gap-4"> <Controller name="purchaseCost" control={control} render={({ field }) => ( <div className="space-y-2"><Label htmlFor="purchase-cost">Total Purchase Cost</Label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input id="purchase-cost" type="number" placeholder="0.00" className="pl-8" {...field} /></div>{methods.formState.errors.purchaseCost && <p className="text-sm text-destructive">{methods.formState.errors.purchaseCost.message}</p>}</div> )}/> <Controller name="initialStock" control={control} render={({ field }) => ( <div className="space-y-2"><Label htmlFor="initial-stock">Number of Containers</Label><Input id="initial-stock" type="number" placeholder="e.g., 12" {...field} />{methods.formState.errors.initialStock && <p className="text-sm text-destructive">{methods.formState.errors.initialStock.message}</p>}</div> )}/> </div>
                        ...
                        {costingMethod === 'uses' && ( <Controller name="usesPerContainer" control={control} render={({ field }) => (<div className="space-y-2"><Label htmlFor="estimated-uses">Est. Uses Per Container</Label><Input id="estimated-uses" type="number" placeholder="e.g., 100 wipes" {...field} /></div>)}/> )}
                    </CardContent>
                    <CardFooter>
