...
const PrintableDocument = ({ d, businessName, onClose }: { d: any; businessName: string; onClose: () => void }) => {
  ...
  let stepN = 0;
  return (
    <div className="print-area fixed inset-0 z-[100] overflow-auto bg-white p-4 sm:p-10">
      <style>{`
        @media print {
        ...
        @page { margin: 0.6in; }
      `}</style>
      <div className="print-hide mb-6 flex flex-wrap items-center justify-between gap-2">
        <button type="button" onClick={() => window.print()} className="rounded-xl bg-slate-900 px-4 py-2 text-[11px] font-black uppercase tracking-widest text-white">
          🖨 Print / Save as PDF
        </button>
      ...
      <CardContent className="p-5 space-y-3">
        ...
        {expandedId === d.id && (
          <>
            ...
            <Button type="button" variant="outline" onClick={onPrint} className="h-auto min-h-10 w-full whitespace-normal rounded-xl border-2 py-2 text-[10px] font-black uppercase leading-tight tracking-widest">
