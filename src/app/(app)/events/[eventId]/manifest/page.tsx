...
const StatCard = ({ label, value, sub, color = 'slate' }: {
  ...
  const colors: Record<string, string> = {
    ...
    emerald: 'bg-emerald-50 border-emerald-200', blue: 'bg-blue-50 border-blue-200', red: 'bg-red-50 border-red-200',
  };
  return (
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="p-4 border-b border-amber-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="font-black text-sm text-amber-800 uppercase tracking-tight">Floor Requests — {requests.length} Pending</p>
          </div>
...
const DeltaRefireBanner = ({ courseNumber, courseName, deltaGuests, onRefire, isFiring }: {
  ...
  <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
    ...
    <div className="flex items-center gap-3 min-w-0">
      <div className="w-9 h-9 rounded-xl bg-indigo-100 flex items-center justify-center shrink-0"><RefreshCw className="w-4 h-4 text-indigo-600" /></div>
      <div className="min-w-0">
        <p className="font-black text-sm text-indigo-900 leading-tight">{deltaGuests.length} new guest{deltaGuests.length !== 1 ? 's' : ''} missed {courseName}</p>
        <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-widest mt-0.5 truncate">{deltaGuests.map(g => g.name).join(', ')}</p>
      </div>
    </div>
...
// ─── BROADCAST PANEL ──────────────────────────────────────────────────────────
const BroadcastPanel = ({ eventId, tenantId, firestore, assignedStaffCount, currentBroadcast }: {
  eventId: string; tenantId: string; firestore: any; assignedStaffCount: number; currentBroadcast: string | null;
}) => {
  ...
  const handleSend = async () => {
    ...
    try {
      await updateDoc(doc(firestore, `tenants/${tenantId}/studioEvents`, eventId), {
        broadcastMessage: message.trim(), broadcastSentAt: new Date().toISOString(), broadcastSentBy: 'host', broadcastDismissed: false,
      });
      setMessage('');
  ...
  const handleClear = async () => {
    ...
    setClearing(true);
    try {
      await updateDoc(doc(firestore, `tenants/${tenantId}/studioEvents`, eventId), { broadcastMessage: null, broadcastSentAt: null, broadcastDismissed: true });
  ...
  return (
    <div className="bg-violet-50 border-2 border-violet-200 rounded-2xl overflow-hidden">
      <div className="p-4 border-b border-violet-200 flex items-center gap-2">
        ...
        {assignedStaffCount > 0 && <span className="ml-auto text-[9px] font-black uppercase tracking-widest text-violet-500">{assignedStaffCount} staff assigned</span>}
      </div>
      {currentBroadcast && (
        <div className="p-4 border-b border-violet-200 flex items-start gap-3">
          <div className="flex-1"><p className="text-[9px] font-black uppercase tracking-widest text-violet-500 mb-1">Active Message</p><p className="text-sm font-bold text-violet-900">"{currentBroadcast}"</p></div>
          <button onClick={handleClear} disabled={clearing} className="shrink-0 w-8 h-8 rounded-xl bg-violet-100 hover:bg-violet-200 flex items-center justify-center transition-all">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {['Mains going out in 5 min', 'Desserts ready shortly', 'Please clear table 3', 'All hands to main floor'].map(preset => (
            <button key={preset} onClick={() => setMessage(preset)} className="text-[9px] font-black uppercase tracking-widest text-violet-600 bg-violet-100 hover:bg-violet-200 px-2.5 py-1.5 rounded-lg transition-colors">{preset}</button>
          ))}
        </div>
      </div>
    </div>
...
// ─── INGREDIENTS LIST ─────────────────────────────────────────────────────────
// Shared component used on menu cards and course firing preview
const IngredientsList = ({ supplies, inventory, guestCount }: {
  supplies: { inventoryId: string; qty: number }[];
  inventory: any[];
  guestCount?: number;
}) => {
  if (!supplies || supplies.length === 0) return (
    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic">No ingredients linked</p>
  );
  return (
    <div className="space-y-1.5">
      {supplies.map((s, i) => {
        const inv = inventory.find((item: any) => item.id === s.inventoryId);
        if (!inv) return null;
        const totalNeeded = guestCount ? s.qty * guestCount : null;
        const inStock = safeNum(inv.totalStock);
        const isShort = totalNeeded !== null && totalNeeded > inStock;
        return (
          <div key={i} className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              {inv.imageUrl
                ? <img src={inv.imageUrl} alt={inv.name} className="w-5 h-5 rounded object-cover shrink-0 border border-slate-200" />
                : <div className="w-5 h-5 rounded bg-slate-100 flex items-center justify-center shrink-0"><Package className="w-3 h-3 text-slate-400" /></div>
              }
              <span className="text-[11px] font-bold text-slate-700 truncate">{inv.name}</span>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[11px] font-black text-slate-900">{s.qty} {inv.unit || 'units'}</span>
              {totalNeeded !== null && (
                <span className={cn('text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-lg',
                  isShort ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700')}>
                  {isShort ? `⚠ Short ${(totalNeeded - inStock).toFixed(1)}` : `✓ ${inStock} in stock`}
                </span>
              )}
              {totalNeeded === null && (
                <span className="text-[9px] font-bold text-slate-400 uppercase">{inStock} in stock</span>
              )}
            </div>
          </div>
        );
...
// ─── MENU ITEM CARD ───────────────────────────────────────────────────────────
// Expandable card with ingredients list + inline add/remove ingredient
const MenuItemCard = ({ item, selectionCount, inventory, tenantId, firestore, onDelete }: {
  item: any; selectionCount: number; inventory: any[];
  tenantId: string; firestore: any; onDelete: (item: any) => void;
}) => {
  const { toast } = useToast();
  ...
  const filteredInv = useMemo(() => {
    const existing = (item.supplies || []).map((s: any) => s.inventoryId);
    const base = (inventory || []).filter((i: any) => !existing.includes(i.id));
    if (!invSearch.trim()) return base.slice(0, 8);
    const s = invSearch.toLowerCase();
    return base.filter((i: any) => i.name?.toLowerCase().includes(s)).slice(0, 8);
  }, [inventory, item.supplies, invSearch]);

  const handleAddIngredient = async (inv: any, qty: number) => {
  ...
  const handleRemoveIngredient = async (inventoryId: string) => {
    if (!firestore || !tenantId) return;
    const newSupplies = (item.supplies || []).filter((s: any) => s.inventoryId !== inventoryId);
    await updateDoc(doc(firestore, `tenants/${tenantId}/eventMenuItems`, item.id), { supplies: newSupplies });
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <button onClick={() => setExpanded(e => !e)}
              className="flex items-center gap-1 mt-1 text-[9px] font-black uppercase tracking-widest text-primary hover:text-primary/80 transition-colors">
              <FlaskConical className="w-2.5 h-2.5" />
              {(item.supplies || []).length} ingredient{(item.supplies || []).length !== 1 ? 's' : ''}
            ...
            <div className="p-4 space-y-3 bg-slate-50/60">
              <div className="flex items-center justify-between">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-1.5">
                  <FlaskConical className="w-3 h-3" /> Ingredients / Formula
                </p>
                <button onClick={() => setShowAddIngredient(s => !s)}
              ...
              {(item.supplies || []).length === 0 ? (
                ...
                <div className="space-y-2">
                  {(item.supplies || []).map((s: any) => {
                    ...
                    if (!inv) return null;
                    return (
                      <div key={s.inventoryId} className="flex items-center gap-3 p-2.5 rounded-xl bg-white border border-slate-200">
                        {inv.imageUrl
                          ? <img src={inv.imageUrl} alt={inv.name} className="w-7 h-7 rounded-lg object-cover shrink-0 border border-slate-200" />
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase">{safeNum(inv.totalStock)} {inv.unit || 'units'} in stock</p>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          />
                          <span className="text-[9px] font-bold text-slate-400 uppercase w-10 truncate">{inv.unit || 'units'}</span>
...
// ─── ADD INGREDIENT ROW ───────────────────────────────────────────────────────
const AddIngredientRow = ({ inv, onAdd, saving }: {
  inv: any; onAdd: (inv: any, qty: number) => Promise<void>; saving: boolean;
}) => {
  const [qty, setQty] = useState(1);
  return (
    <div className="flex items-center gap-2 p-2 rounded-xl border border-slate-200 bg-white hover:border-primary/30 transition-colors">
      {inv.imageUrl
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <span className="text-[9px] font-bold text-slate-400 uppercase w-8 truncate">{inv.unit || 'ea'}</span>
...
// Shows what stock will be deducted when this course fires
const CourseIngredientsPreview = ({ courseNumber, menuItems, guests, inventory }: {
  ...
  const deductionSummary = useMemo(() => {
    const map: Record<string, { name: string; qty: number; unit: string; inStock: number; imageUrl?: string }> = {};
    const checkedIn = guests.filter(g => g.checkedIn && (g.courseSelections?.[courseNumber] || (courseNumber === 1 && g.mealChoiceId)));
    checkedIn.forEach(guest => {
      ...
      menuItem.supplies.forEach((s: any) => {
        const inv = (inventory || []).find((i: any) => i.id === s.inventoryId);
        if (!inv) return;
        if (!map[s.inventoryId]) map[s.inventoryId] = { name: inv.name, qty: 0, unit: inv.unit || 'units', inStock: safeNum(inv.totalStock), imageUrl: inv.imageUrl };
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="space-y-1.5 p-3 rounded-xl bg-slate-50 border border-slate-200">
              <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-2">Stock deducted when fired</p>
...
export default function EventManifestPage() {
  ...
  // clientSearch declared above — safe to use here
    ...
    guests.forEach(guest => {
      ...
      items.forEach(item => {
        (item.supplies || []).forEach((s: any) => {
          const inv = (inventory || []).find((i: any) => i.id === s.inventoryId);
          if (!inv) return;
          if (!supplyNeeds[s.inventoryId]) supplyNeeds[s.inventoryId] = { name: (inv as any).name, needed: 0, inStock: safeNum((inv as any).totalStock), unit: (inv as any).unit || 'units', status: 'ok' };
  ...
  const generateQRDataUrl = async (url: string): Promise<string> => `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(url)}`;
  ...
  const handlePrintQRs = () => {
    ...
    area.querySelectorAll('.flex.flex-col').forEach(card => { const img = card.querySelector('img') as HTMLImageElement; const label = card.querySelector('p')?.textContent || ''; win.document.write(`<div class="card"><img src="${img?.src}" /><p>${label}</p></div>`); });
  ...
  const handleFireCourse = async (courseNumber: number) => {
    ...
    if (courseNumber > 1) { const unfiredPrev = courseNumbers.filter(n => n < courseNumber && !firedCourses.has(n)); if (unfiredPrev.length > 0) { const ok = window.confirm(`Course ${unfiredPrev.join(', ')} not fired yet. Continue?`); if (!ok) return; } }
  ...
  const handleCheckInGuest = async (guestId: string, currentValue: boolean) => {
    if (!firestore || !tenantId) return;
    await updateDoc(doc(firestore, `tenants/${tenantId}/eventGuests`, guestId), { checkedIn: !currentValue, checkedInAt: !currentValue ? new Date().toISOString() : null });
  ...
  const handleImportClient = async (client: any) => {
    ...
    const id = nanoid();
    await addDoc(collection(firestore, `tenants/${tenantId}/eventGuests`), { id, eventId, tenantId, name: client.name, email: client.email || '', phone: client.phone || '', tableNumber: '', seatNumber: '', mealChoiceId: null, mealChoiceName: null, allergies: [], dietaryRestrictions: [], checkedIn: false, source: 'client_import', clientId: client.id, submittedAt: new Date().toISOString() });
  ...
  const handleLinkInventoryItem = (invItem: any) => {
    ...
    if (!menuSupplies.find(s => s.inventoryId === invItem.id)) setMenuSupplies(prev => [...prev, { inventoryId: invItem.id, qty: 1 }]);
  };
  ...
  const handleDeleteMenuItem = async (item: any) => {
    ...
    if (selectCount > 0) { const ok = window.confirm(`${selectCount} guest${selectCount !== 1 ? 's have' : ' has'} selected "${item.name}". Delete anyway?`); if (!ok) return; const batch = writeBatch(firestore); guests.filter(g => g.mealChoiceId === item.id).forEach(g => { batch.update(doc(firestore, `tenants/${tenantId}/eventGuests`, g.id), { mealChoiceId: null, mealChoiceName: null, mealClearedReason: `Menu item "${item.name}" was deleted` }); }); await batch.commit(); }
  ...
  const handleMealOverride = async () => {
    ...
    await updateDoc(doc(firestore, `tenants/${tenantId}/eventGuests`, mealOverrideGuest.id), { mealChoiceId: resolvedId, mealChoiceName: mealItem?.name || null, mealOverriddenAt: new Date().toISOString(), mealOverriddenBy: 'staff' });
    setSavingOverride(false); setMealOverrideGuest(null); setMealOverrideId(''); toast({ title: `Meal updated for ${mealOverrideGuest.name}` });
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-50">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 pb-24">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {event?.status === 'active' && (
              <Button variant="outline" onClick={() => setShowBroadcast(s => !s)}
                className={cn('h-10 rounded-xl border-2 font-black uppercase text-[10px] tracking-widest gap-2', currentBroadcast ? 'border-violet-300 bg-violet-50 text-violet-700' : '')}>
          ...
          {undoWindowOpen && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-black text-sm text-emerald-800">Event is now live — kiosk switched to event mode</p><p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Undo available for {undoCountdown}s</p></div>
              </div>
        ...
        <Dialog open={isConfirmActivateOpen} onOpenChange={setIsConfirmActivateOpen}>
          <DialogContent className="sm:max-w-md rounded-[2rem] border-4 shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2"><Button variant="outline" onClick={() => setIsConfirmActivateOpen(false)} className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest border-2">Cancel</Button><Button onClick={handleActivateEvent} disabled={activatingNow} className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-200 gap-2">{activatingNow ? <Loader className="w-4 h-4 animate-spin" /> : '🟢 Activate Event'}</Button></div>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isEndEventOpen} onOpenChange={setIsEndEventOpen}>
          <DialogContent className="sm:max-w-md rounded-[2rem] border-4 shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Event Summary</p><p className="font-black text-slate-900">{eventDisplayName}</p><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{stats.checkedIn} of {stats.total} guests attended · {fires.filter(f => f.status === 'fired' && !f.isDelta).length} of {courseNumbers.length} courses fired</p></div>
              ...
              <div className="flex gap-3 pt-2"><Button variant="outline" onClick={() => setIsEndEventOpen(false)} className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest border-2">Cancel</Button><Button onClick={handleConfirmEndEvent} className="flex-1 h-12 rounded-2xl font-black uppercase text-[10px] tracking-widest bg-slate-800 hover:bg-slate-900 gap-2">End Event →</Button></div>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <AnimatePresence>
          {showLink && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-white rounded-2xl border-2 border-primary/20 p-5 space-y-3">
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Guest Order Link</p>
              <div className="flex items-center gap-3"><code className="flex-1 text-xs font-bold bg-slate-50 rounded-xl px-4 py-3 border-2 border-slate-200 truncate text-slate-700">{shareableLink}</code><Button onClick={copyLink} className="h-11 px-4 rounded-xl font-black uppercase text-[10px] tracking-widest gap-2 shrink-0"><Copy className="w-4 h-4" /> Copy</Button></div>
              <div className="mt-4 space-y-3">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Generate Per-Seat QR Codes</p>
                <div className="flex items-center gap-3"><Input placeholder="Tables (e.g. 1,2,3)" value={qrTables} onChange={e => setQrTables(e.target.value)} className="h-10 rounded-xl border-2 flex-1" /><Input placeholder="Seats per table" value={qrSeatsPerTable} onChange={e => setQrSeatsPerTable(e.target.value)} className="h-10 rounded-xl border-2 w-40" /><Button onClick={handleGenerateQRs} className="h-10 px-4 rounded-xl font-black uppercase text-[10px] tracking-widest gap-2 shrink-0"><QrCode className="w-4 h-4" /> Generate</Button></div>
                {qrCodes.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-3 max-h-64 overflow-y-auto" id="qr-print-area">{qrCodes.map(qr => (<div key={qr.label} className="flex flex-col items-center gap-1 p-3 border-2 rounded-xl bg-white"><img src={qr.dataUrl} alt={qr.label} className="w-16 h-16" /><p className="text-[8px] font-black uppercase text-slate-600 text-center">{qr.label}</p></div>))}</div>
                  </div>
                )}
              </div>
            </motion.div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="guests" className="mt-4 space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {stats.notCheckedIn > 0 && filterFlag !== 'not-checked-in' && (
                <button onClick={() => setFilterFlag('not-checked-in')} className="flex items-center gap-1.5 h-10 px-3 rounded-xl border-2 border-amber-200 bg-amber-50 text-amber-700 font-black uppercase text-[9px] tracking-widest hover:bg-amber-100 transition-all"><AlertTriangle className="w-3 h-3" /> {stats.notCheckedIn} Not In</button>
              )}
            </div>
                    ...
                    {!editingGuest && clientSearch && filteredClients.length > 0 && (
                      <div className="rounded-xl border-2 divide-y overflow-hidden">
                        {filteredClients.map((c: any) => (
                          <button key={c.id} onClick={() => { handleImportClient(c); setClientSearch(''); }} className="w-full flex items-center justify-between p-3 hover:bg-primary/5 transition-colors text-left gap-3">
          ...
          <TabsContent value="menu" className="mt-4 space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        {linkedInvItem ? (
                          <div className="flex items-center gap-2 p-3 rounded-xl bg-primary/5 border-2 border-primary/20">
                            ...
                            <div className="flex-1 min-w-0"><p className="font-black text-sm text-slate-900 truncate">{(linkedInvItem as any).name}</p><p className="text-[9px] font-bold text-slate-500 uppercase">{(linkedInvItem as any).unit || 'units'} · Stock: {safeNum((linkedInvItem as any).totalStock)}</p></div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                            {filteredInventory.length === 0
                              ? <p className="col-span-3 text-center text-[10px] font-bold text-slate-400 py-4 uppercase tracking-widest">{(inventory || []).length === 0 ? 'No inventory items yet' : 'No results'}</p>
                              : filteredInventory.map((inv: any) => (
                                <button key={inv.id} onClick={() => handleLinkInventoryItem(inv)} className="flex items-center gap-2 p-2.5 rounded-xl border-2 border-slate-200 hover:border-primary/40 hover:bg-primary/5 transition-all text-left">
                                  ...
                                  <div className="min-w-0"><p className="font-black text-[11px] text-slate-900 truncate">{inv.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase">{inv.unit || 'units'} · {safeNum(inv.totalStock)}</p></div>
                                </button>
                              ))
                            }
                          </div>
                      ...
                      <div className="border-t border-slate-100 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        {menuSupplies.length > 0 && (
                          <div className="sm:col-span-2 space-y-2">
                            ...
                            {menuSupplies.map((s, idx) => { const inv = (inventory || []).find((i: any) => i.id === s.inventoryId); if (!inv) return null; return (<div key={s.inventoryId} className="flex items-center gap-3 p-3 rounded-xl border-2 border-slate-200 bg-slate-50"><p className="flex-1 font-black text-sm text-slate-900">{(inv as any).name}</p><div className="flex items-center gap-2"><Input type="number" min="0.1" step="0.1" value={s.qty} onChange={e => setMenuSupplies(prev => prev.map((sp, i) => i === idx ? { ...sp, qty: parseFloat(e.target.value) || 1 } : sp))} className="w-20 h-9 rounded-xl border-2 text-center font-bold text-sm" /><span className="text-[10px] font-bold text-slate-400 uppercase">{(inv as any).unit || 'units'}</span></div><button onClick={() => setMenuSupplies(prev => prev.filter((_, i) => i !== idx))} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors"><X className="w-3.5 h-3.5" /></button></div>); })}
                          </div>
                        )}
                      </div>
            ...
            {menuItems.length === 0 && !isAddingMenu && (
              <div className="text-center py-10 border-2 border-dashed rounded-3xl"><Utensils className="w-8 h-8 text-slate-300 mx-auto mb-3" /><p className="font-black uppercase text-[10px] tracking-widest text-slate-400">No menu items yet</p></div>
            )}
          </TabsContent>
                ...
                {(event?.assignedStaffIds || []).map((staffId: string) => {
                  ...
                  return (
                    <div key={staffId} className="flex items-center justify-between p-3 rounded-2xl border-2 border-slate-200">
                      <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center font-black text-primary text-sm">{(member as any).name?.charAt(0)}</div><div><p className="font-black text-sm text-slate-900">{(member as any).name}</p><p className="text-[9px] font-bold uppercase text-slate-400">{(member as any).role}</p></div></div>
