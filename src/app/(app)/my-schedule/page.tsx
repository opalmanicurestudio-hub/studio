...
export default function MySchedulePage() {
  ...
  // Simple query -- only filter by staffId to avoid composite index requirement
  // All other filtering done client-side
  ...
  // Simple query -- only filter by staffId
      ...
      <main className="flex-1 p-4 md:p-8 w-full max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">
              Your shifts & requests
            </p>
          </div>
          <Button
            onClick={() => setIsRequestOpen(true)}
            className="h-12 px-5 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-primary/20"
          >
        ...
        <div className="flex items-center justify-between gap-4 p-4 bg-white rounded-[2rem] border-2 shadow-sm">
          ...
          <div className="text-center">
            ...
            <p className="text-[9px] font-black uppercase text-muted-foreground opacity-60 mt-0.5">
              {weeklyHours.toFixed(1)}h scheduled
            </p>
          </div>
          ...
          {weekDays.map(day => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={cn(
                  "rounded-[2rem] border-2 overflow-hidden transition-all bg-white",
                  isToday(day) ? "border-primary/30 shadow-md shadow-primary/10" : "border-slate-200",
                  isPast && "opacity-50"
                )}
              >
                {/* Day header */}
                <div className={cn(
                  "px-5 py-3 flex items-center justify-between border-b border-dashed",
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={cn(
                        "font-black uppercase text-sm leading-none",
                  ...
                  {isToday(day) && (
                    <Badge className="bg-primary text-white border-none font-black text-[9px] uppercase">Today</Badge>
                  )}
                </div>
                  ...
                  {dayShifts.length > 0 ? dayShifts.map(shift => (
                    <div
                      ...
                      className={cn(
                        ...
                        STATUS_COLORS[shift.status] || STATUS_COLORS.published
                      )}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {shift.notes && (
                            <p className="text-[9px] font-bold italic opacity-70 mt-0.5 truncate">{shift.notes}</p>
                          )}
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-3">
                        <p className="font-black font-mono text-sm text-primary">
                          {calcHours(shift.startTime, shift.endTime, shift.breakMinutes || 0).toFixed(1)}h
                        </p>
                        <Badge className={cn(
                    ...
                    <p className="text-[9px] font-black uppercase text-muted-foreground opacity-40 text-center py-2">
                      No shift scheduled
                    </p>
                  )}
                </div>
            ...
            {myRequests.slice(0, 5).map(req => (
              <Card key={req.id} className="border-2 rounded-[2rem] shadow-sm bg-white">
                <CardContent className="p-4 flex items-start gap-4">
                  <div className={cn(
                    "p-2.5 rounded-xl shrink-0 border-2",
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <p className="font-black uppercase text-[10px] text-slate-900">
                        {req.type === 'day_off' ? 'Day Off' :
                         req.type === 'swap' ? 'Shift Swap' : 'Early Release'}
                        {req.date && ` -- ${format(safeDate(req.date), 'MMM d')}`}
                      </p>
                      <Badge className={cn(
                        "font-black text-[8px] uppercase border-none h-5 px-2 shrink-0",
                    ...
                    <p className="text-[9px] font-bold text-muted-foreground uppercase opacity-60 mt-0.5 truncate">
                      {req.reason}
                    </p>
                    {req.managerNote && (
                      <p className="text-[9px] font-bold text-primary/70 italic mt-1 border-l-2 border-primary/20 pl-2">
                        {req.managerNote}
                      </p>
                    )}
                  </div>
        ...
        <DialogContent className="sm:max-w-md rounded-[3rem] border-4 shadow-3xl bg-background">
          ...
          <div className="p-6 space-y-5">
            <div className="space-y-2">
              ...
              <Select value={requestType} onValueChange={(v: any) => setRequestType(v)}>
                <SelectTrigger className="h-12 rounded-2xl border-2 font-black uppercase text-[10px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="rounded-xl border-2 shadow-2xl">
                  <SelectItem value="day_off" className="font-bold uppercase text-[10px]">
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-blue-600" /> Day Off Request
                    </div>
                  </SelectItem>
                  <SelectItem value="swap" className="font-bold uppercase text-[10px]">
                    <div className="flex items-center gap-2">
                      <Repeat className="w-4 h-4 text-purple-600" /> Shift Swap
                    </div>
                  </SelectItem>
                  <SelectItem value="early_release" className="font-bold uppercase text-[10px]">
                    <div className="flex items-center gap-2">
                      <Zap className="w-4 h-4 text-amber-600" /> Early Release
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          <DialogFooter className="p-6 pt-0 flex flex-col gap-3">
            <Button
              ...
              disabled={isProcessing || !requestDate || !requestReason.trim()}
