              ...
              {tab === 'curriculum' && (
                <div className="space-y-4">
                  <CourseBoard lessons={d.lessons || []} kindIcon={(k) => KIND_ICON[k] || FileText}
                    ...
                    lessonActions={(l) => <><button type="button" aria-label="Edit" onClick={() => setLesson({ ...blankLesson(), ...l, videoUrl: l.videoUrl || '', downloadUrl: l.downloadUrl || '', downloadName: l.downloadName || '', minMinutes: l.minMinutes || 0, releaseAfterDays: l.releaseAfterDays || 0, quiz: l.quiz || null, flashcards: l.flashcards || [], activity: l.activity || null, blocks: l.blocks || [], layout: l.layout || null, stepMode: l.stepMode === true, plan: l.plan || null, assignment: l.assignment || null, cases: l.cases || null, videoQuestions: l.videoQuestions || [], transcript: l.transcript || null })} className="p-1"><Pencil className="h-4 w-4" /></button><button type="button" aria-label="Delete" onClick={async () => { if (window.confirm(`Delete “${l.title}”?`)) { await api({ action: 'lesson-delete', tenantId, courseId: sel, lessonId: l.id }); await loadCourse(sel!); } }} className="p-1 text-red-600"><Trash2 className="h-4 w-4" /></button></>}
                  ...
                  {fromFile && <FileToLesson call={(body) => api({ ...body, tenantId, courseId: sel })} onClose={() => setFromFile(null)} onDraft={(dr) => { const mod = fromFile; setFromFile(null); setLesson({ ...blankLesson(mod), layout: 'article', ...dr }); setMsg('Here’s the draft from your file — check it, change anything, then Save.'); }} />}
                  ...
                  {lesson && (
                    <div className="fixed inset-0 z-50 overflow-y-auto bg-background">
                      ...
                      <aside className="hidden lg:block"><div className="sticky top-20 space-y-2"><p className="text-center text-[11px] font-black uppercase tracking-widest text-muted-foreground">As a student sees it</p><LessonPreview lesson={lesson} color={docBrand.color} stepMode={layoutOf(lesson) === 'steps'} /></div></aside>
                      </div>
                  ...
                  {addOpen && <AddToLesson lesson={lesson} setLesson={setLesson} draft={draft} drafting={drafting} onClose={() => setAddOpen(false)} />}
                      {showPrev && <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center gap-3 bg-black/70 p-4" onClick={() => setShowPrev(false)}><div onClick={(e) => e.stopPropagation()}><LessonPreview lesson={lesson} color={docBrand.color} stepMode={layoutOf(lesson) === 'steps'} /></div><button type="button" className="h-10 rounded-full bg-white px-5 text-sm font-bold">Close preview</button></div>}
                      {drawer && (
                      ...
                      <div className="flex items-center justify-between"><p className="text-xl font-black">Lesson settings</p><button type="button" onClick={() => setDrawer(false)} className="text-sm font-bold text-muted-foreground">Done</button></div>
                            <LayoutPicker lesson={lesson} setLesson={setLesson} onApplyAll={lesson.id ? async (layout) => { const r = await api({ action: 'lessons-layout', tenantId, courseId: sel, layout }); if (!r.ok) return r.error; await loadCourse(sel!); return null; } : undefined} />
