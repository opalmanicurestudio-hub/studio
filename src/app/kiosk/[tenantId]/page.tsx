...
const btnStyle = (primaryHex?: string, base?: KioskTheme): React.CSSProperties | undefined => {
  ...
  if (base === 'dark' || base === 'slate') return undefined;
...
const StepDots = ({ total, current, t, primaryHex }: { total: number; current: number; t: ThemeTokens; primaryHex?: string }) => (
  <div className="flex items-center gap-2">
    {Array.from({ length: total }).map((_, i) => (
      <div key={i}
        className={cn('h-1.5 rounded-full transition-all duration-500', i < current ? 'w-3 opacity-40' : 'w-3 opacity-20')}
        style={{
          width: i === current ? '1.5rem' : undefined,
          backgroundColor: i <= current ? (primaryHex || (t.text === 'text-white' ? '#fff' : '#0f172a')) : undefined,
    ...
    <div className="p-8 md:p-12 space-y-8">
      ...
      <div className="space-y-3">
        <button onClick={() => onSelect(size)}
          ...
          Continue with {size} guests &rarr;
        </button>
        <button onClick={onBack} className={cn('w-full text-center text-[10px] font-black uppercase tracking-widest', t.muted, 'hover:opacity-70 transition-opacity')}>&larr; Back</button>
      </div>
    </div>
  );
};
          ...
          <div key={member.id} className={cn('p-5 rounded-2xl border-2 space-y-4', t.card, t.cardBorder)}>
            ...
            <div className="space-y-1.5">
              <label className={cn('text-[9px] font-black uppercase tracking-[0.2em] flex items-center gap-1.5', t.label)}>
                <User className="w-3 h-3" /> Full Name (required)
              </label>
              <input
                value={member.name}
  ...
  <div className="space-y-4">
    <div className="space-y-2">
      ...
      <PhoneInput
        ...
        placeholder="(555) 000-0000"
        className={cn('h-14 w-full rounded-2xl border-2 px-4 text-lg font-bold transition-all', t.inputBg, t.inputBorder, t.text, '[&_input]:border-none [&_input]:bg-transparent [&_input]:placeholder:opacity-30 focus-within:ring-2')}
      />
    </div>
    <div className="space-y-2">
      ...
      <input
        ...
        className={cn('w-full h-14 rounded-2xl border-2 px-4 text-lg font-bold outline-none transition-all uppercase tracking-tight', t.inputBg, t.inputBorder, t.text, 'placeholder:opacity-30 focus:ring-2')}
      />
    </div>
    ...
    <div className="space-y-2">
      ...
      <input
        ...
        className={cn('w-full h-14 rounded-2xl border-2 px-4 text-lg font-bold outline-none transition-all', t.inputBg, t.inputBorder, t.text, 'placeholder:opacity-30 focus:ring-2')}
      />
    </div>
    <AnimatePresence>
      {isResolvingIdentity && (
      ...
      {isKnownClient && clientType === 'new' && !bannedClient && !existingClientWithBalance && (
        <motion.div key="known" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div>
            ...
            <p className="text-[9px] font-bold text-amber-600 uppercase leading-relaxed">We found a record. Please go back and choose <strong>Return Guest</strong>.</p>
          </div>
        </motion.div>
      )}
      {bannedClient && (
        <motion.div key="banned" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div><p className="text-[10px] font-black uppercase tracking-widest text-red-600 mb-1">Access Restricted</p>
          <p className="text-[9px] font-bold text-red-500 uppercase">{bannedClient.banMessage || 'Please see front desk.'}</p></div>
        </motion.div>
      )}
      {existingClientWithBalance && !bannedClient && (
        <motion.div key="balance" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div><p className="text-[10px] font-black uppercase tracking-widest text-red-600 mb-1">Balance Due</p>
          <p className="text-[9px] font-bold text-red-500 uppercase">${existingClientWithBalance.outstandingBalance?.toFixed(2)} - settle at desk first.</p></div>
        </motion.div>
      )}
    </AnimatePresence>
  </div>
  ...
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <button onClick={() => onUpdate({ preferredStaffId: 'any' })}
        className={cn('flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all', (!member.preferredStaffId || member.preferredStaffId === 'any') ? 'border-transparent shadow-md' : `${t.card} ${t.cardBorder} ${t.text}`)}
        style={(!member.preferredStaffId || member.preferredStaffId === 'any') ? (btnStyle(primaryHex) || {}) : undefined}>
        <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center', 'bg-white/20')}><Users className="w-6 h-6" strokeWidth={1.3} /></div>
      ...
      {(staff || []).map((s: Staff) => (
        <button key={s.id} onClick={() => onUpdate({ preferredStaffId: s.id })}
          ...
          style={member.preferredStaffId === s.id ? (btnStyle(primaryHex) || {}) : undefined}>
    ...
    <div className="p-6 md:p-8 space-y-6">
      ...
      {memberSubStep === 'details' && matchedAppointment && (
        <div className={cn('p-4 rounded-2xl border-2 space-y-3', t.card, t.cardBorder)}>
          ...
          <button onClick={() => onAppointmentCheckIn(matchedAppointment)}
            ...
            style={btnStyle(primaryHex)}>
            Check In for This Appointment &rarr;
          </button>
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={onBack} disabled={isSubmitting}
          className={cn('h-14 px-5 rounded-2xl border-2 font-black uppercase text-sm transition-all', t.card, t.cardBorder, t.muted, 'hover:opacity-70')}>&larr;</button>
  ...
  <div className="p-8 md:p-10 space-y-8">
    <div className="text-center space-y-3">
      ...
      <p className={cn('font-bold uppercase tracking-[0.15em] text-sm', t.muted)}>
        {confirmedParty.length > 1 ? `${confirmedParty.length} tickets ready - print one for each guest` : "We'll notify you when it's your turn"}
      </p>
    </div>
...
const BirthdayCelebrationView = ({ clientName, onDone, t, primaryHex }: any) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <div className="relative z-10 space-y-8 max-w-sm">
      ...
      <button onClick={onDone} className={cn('h-14 px-10 rounded-full font-black uppercase tracking-widest', t.btn, t.btnText)} style={btnStyle(primaryHex)}>Continue &rarr;</button>
    </div>
  </motion.div>
);
                        ...
                        <div className="p-8 md:p-12 space-y-8">
                          ...
                          <div className="text-center">
                            <button onClick={() => setStep(isGroup ? 'guestNames' : 'partyType')}
                              className={cn('text-[10px] font-black uppercase tracking-widest', t.muted, 'hover:opacity-70 transition-opacity')}>&larr; Back</button>
                          </div>
                        </div>
                      </SurfaceCard></motion.div>
                    )}
                      ...
                      <motion.div key="phone"><SurfaceCard t={t}>
                        <div className="p-8 md:p-12 space-y-8">
                          ...
                          <div className="space-y-3 max-w-xs mx-auto">
                            ...
                            <button onClick={() => setStep('identityChoice')} className={cn('w-full text-center text-[10px] font-black uppercase tracking-widest', t.muted, 'hover:opacity-70')}>&larr; Back</button>
                          </div>
                        </div>
                      </SurfaceCard></motion.div>
                    )}
