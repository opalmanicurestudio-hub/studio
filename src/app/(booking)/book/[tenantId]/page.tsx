...
// Convert hex color to HSL string for CSS variable injection (shadcn uses HSL)
...
const entrance=(name:string,dur=700,delay=0):React.CSSProperties=>({
...
function HeroSection({config,style,isPreview,sectionId,onFieldTap}:SectionProps){
  ...
  const Headline=()=>(
    <FieldTap sectionId={sectionId} fieldKey="headline" isPreview={isPreview} onFieldTap={onFieldTap}
      ...
      style={{fontFamily:hf(style),color:tc,...entrance('cf-fade-up',900,100)}}>
  ...
  const Sub=()=>(
    <FieldTap sectionId={sectionId} fieldKey="subheadline" isPreview={isPreview} onFieldTap={onFieldTap}
      ...
      style={{fontFamily:bf(style),color:sc,...entrance('cf-fade-up',900,280)}}>
      {config.subheadline}
    </FieldTap>
  );
  const Btns=()=>(
    <div className={cn('flex flex-wrap gap-4',!isSplit&&'justify-center')} style={entrance('cf-fade-up',900,440)}>
      <FieldTap sectionId={sectionId} fieldKey="ctaText" isPreview={isPreview} onFieldTap={onFieldTap} as="span">
  ...
  return(
    <section className="relative flex items-center overflow-hidden" style={{minHeight:isFullbleed?'100vh':'82vh',background:hasBg&&!isMinimal?`url(${config.bgImage}) center/cover no-repeat`:isMinimal?'#ffffff':style.bgColor}}>
      ...
      {config.showBadge&&config.badgeText&&<FieldTap sectionId={sectionId} fieldKey="badgeText" isPreview={isPreview} onFieldTap={onFieldTap} as="div" className="absolute top-6 left-1/2 -translate-x-1/2 z-10 px-5 py-1.5 border border-white/30 bg-white/10 backdrop-blur text-white text-[10px] font-black uppercase tracking-widest rounded-full" style={entrance('cf-fade-in',600,200)}>{config.badgeText}</FieldTap>}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-16 py-24">
        {isSplit?(
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div style={entrance('cf-scale-up',900,200)}>
...
function ServicesSection({config,style,data,isPreview,sectionId,onFieldTap}:SectionProps){
  ...
  const animN=ANIM_MAP[(config._animation?.type)||'fade-up']||'cf-fade-up';
  const animD=config._animation?.speed||600;
  const sa=(i:number)=>({animationName:animN,animationDuration:`${animD}ms`,animationDelay:`${i*80}ms`,animationFillMode:'both' as const,animationTimingFunction:'cubic-bezier(0.16,1,0.3,1)'});
  return(
          ...
          layout==='list'?(
            <div className="space-y-3 max-w-3xl mx-auto">
              {services.map((svc:any,i:number)=>(
                <div key={svc.id} className="flex items-center gap-5 p-5 bg-white hover:shadow-md transition-all" style={{borderRadius:br(style),border:`2px solid ${ac(style)}20`,...sa(i)}}>
            ...
            <div className={`grid gap-5 ${gridCls}`}>
              {services.map((svc:any,i:number)=>(
                <div key={svc.id} className={cn('group p-7 bg-white transition-all duration-300',config.hoverEffect!==false&&'hover:shadow-xl hover:-translate-y-1')} style={{borderRadius:br(style,1.5),border:`2px solid ${ac(style)}25`,...sa(i)}}>
...
function TeamSection({config,style,data,isPreview,sectionId,onFieldTap}:SectionProps){
  ...
  const animN=ANIM_MAP[(config._animation?.type)||'scale-up']||'cf-scale-up';
  const animD=config._animation?.speed||600;
  const ma=(i:number)=>({animationName:animN,animationDuration:`${animD}ms`,animationDelay:`${i*80}ms`,animationFillMode:'both' as const,animationTimingFunction:'cubic-bezier(0.16,1,0.3,1)'});
...
function ReviewsSection({config,style,isPreview,sectionId,onFieldTap}:SectionProps){
  ...
  const reviews=[1,2,3,4,5,6].map(n=>({
    ...
    text:config[`rev${n}Text`]||['Absolutely incredible experience. The attention to detail is unmatched — I leave feeling taken care of every single time.',"I've been coming here for over a year and every visit exceeds my expectations.",'The atmosphere is luxurious yet so welcoming. I always feel like a VIP.','Cannot recommend enough. Talented, professional, and truly exceptional.','A gem of a studio. Every appointment is a joy. My nails have never looked better.','So glad I found this place. Creative, professional, and always on time.'][n-1]||'',
...
function BookingPageContent({tenantId}:{tenantId:string}){
  ...
  const activeStyle:StyleConfig={
    accentColor:  liveConfig?.style?.accentColor  ??savedConfig?.accentColor  ??DS.accentColor,
    bgColor:      liveConfig?.style?.bgColor      ??savedConfig?.bgColor      ??DS.bgColor,
    headingFont:  liveConfig?.style?.headingFont  ??savedConfig?.headingFont  ??DS.headingFont,
    bodyFont:     liveConfig?.style?.bodyFont     ??savedConfig?.bodyFont     ??DS.bodyFont,
    borderRadius: liveConfig?.style?.borderRadius ??savedConfig?.borderRadius ??DS.borderRadius,
    buttonStyle:  liveConfig?.style?.buttonStyle  ??savedConfig?.buttonStyle  ??DS.buttonStyle,
    density:      liveConfig?.style?.density      ??savedConfig?.density      ??DS.density,
  };
  ...
  // Inject CSS variables so BookingSheet inherits the booking page theme
  useEffect(()=>{
    const root=document.documentElement;
    root.style.setProperty('--booking-heading-font',hf(activeStyle));
    root.style.setProperty('--booking-body-font',bf(activeStyle));
    root.style.setProperty('--radius',`${activeStyle.borderRadius}px`);
    // Map accent color → shadcn --primary (HSL format)
    try{ root.style.setProperty('--primary',hexToHsl(activeStyle.accentColor)); }catch{}
  },[activeStyle]);

  // onConfirm handler — writes a pending booking request (not a confirmed appointment)
  ...
  return(
    <div className="w-full min-h-dvh overflow-x-hidden" style={{background:activeStyle.bgColor,fontFamily:bf(activeStyle)}}>
      {isPreview&&(
      ...
      {/* Service picker — shown when Book Now clicked without a pre-selected service */}
      {showPicker&&(
        <div className="fixed inset-0 z-[300] flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-lg sm:mx-4 bg-white overflow-hidden" style={{borderRadius:'24px 24px 0 0',maxHeight:'80dvh'}}>
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{borderColor:ac(activeStyle)+'20'}}>
              <p className="font-black text-sm uppercase tracking-widest" style={{fontFamily:bf(activeStyle),color:ac(activeStyle)}}>Select a Service</p>
            ...
            <div className="overflow-y-auto p-4 space-y-2" style={{maxHeight:'60dvh'}}>
              {services.map((s:any)=>(
                <button key={s.id} onClick={()=>{setDialogService(s);setShowPicker(false);setDialogOpen(true);}}
                  ...
                  style={{borderRadius:br(activeStyle),border:`2px solid ${ac(activeStyle)}25`,background:'white'}}>
                  <div className="flex-1 min-w-0">
                    <p className="font-black text-sm uppercase tracking-tight text-slate-900 truncate" style={{fontFamily:bf(activeStyle)}}>{s.name}</p>
                    {s.duration&&<p className="text-[10px] font-black uppercase tracking-widest mt-0.5" style={{color:ac(activeStyle)+'80'}}>{s.duration} min</p>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-4">
                    {s.price&&<span className="text-xl font-light" style={{fontFamily:hf(activeStyle),color:ac(activeStyle)}}>${s.price}</span>}
      ...
      {/* BookingSheet — full production booking flow, themed via CSS variables */}
