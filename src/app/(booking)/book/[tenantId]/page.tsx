...
// Simple check — trust what the builder saved
...
function ReviewsSection({ config, style, isPreview, sectionId, onFieldTap }: SectionProps) {
  ...
  const reviews = [1,2,3,4,5,6].map(n => ({
    ...
    text:   config[`rev${n}Text`]   || ['Absolutely incredible experience.','Every visit exceeds my expectations.','Luxurious yet so welcoming.','Cannot recommend enough.','A gem of a studio.','Creative and professional.'][n-1] || '',
...
function BookingPageContent({ tenantId }: { tenantId: string }) {
  ...
  // savedConfig: from Firestore — source of truth for the public page
  const [savedConfig,setSavedConfig] = useState<PageBuilderConfig|null>(null);
  // liveConfig: real-time updates from the page builder iframe (preview only)
  const [liveConfig,setLiveConfig]   = useState<{ sections: PageSection[]; style: any }|null>(null);

  // ── configReady is the ONLY gate for the spinner ──────────────────────────
  // We do NOT gate on liveReady. The preview gets live updates reactively
  // once CLARITY_PREVIEW arrives, without blocking the initial render.
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled && tSnap.exists()) {
          ...
          console.log('[booking] cfPageConfig:', pc ? `found (${pc.sections?.length ?? 0} sections)` : 'not found');
          if (isBuilderConfig(pc)) {
            setSavedConfig(pc as PageBuilderConfig);
            console.log('[booking] ✓ savedConfig loaded');
          } else {
            console.warn('[booking] no valid cfPageConfig — using defaults');
          }
        }
      } catch (e) {
  ...
  // Preview: receive live config from the page builder (reactive, non-blocking)
  useEffect(() => {
  ...
  // Preview: signal builder that this iframe is ready to receive config.
  // Best-effort — the builder ALSO sends CLARITY_PREVIEW proactively on a
  // 400ms debounce, so missing the handshake is not fatal.
  useEffect(() => {
    if (!isPreview) return;
    const send = () => { try { window.parent.postMessage({ type: 'BOOKING_READY' }, '*'); } catch {} };
    send();
    const t = setInterval(send, 500);
    // Stop retrying after 10s — the builder's debounced sync will cover it
  ...
  const handleFieldTap = useCallback((sectionId: string, fieldKey: string) => {
    if (isPreview) { try { window.parent.postMessage({ type: 'EDIT_FIELD', sectionId, fieldKey }, '*'); } catch {} }
  }, [isPreview]);
  ...
  // Resolve config:
  // - Public page: always savedConfig (liveConfig is never set)
  // - Preview iframe: liveConfig overlays savedConfig for instant updates
  ...
  // ── Spinner: only blocks on configReady, never on liveReady ───────────────
  if (!configReady) {
    return (
