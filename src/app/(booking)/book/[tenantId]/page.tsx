...
import type {
  Staff, Service, Appointment, Event, ConsentForm,
  Tenant, Client, Membership, Package, PricingTier,
} from '@/lib/data';
import {
  Loader, ArrowDown, Users, Sparkles, MapPin, Phone,
  Instagram, ArrowRight, FileText, Calendar, Star,
    ...
    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-12 lg:gap-16">
      ...
      <div className="flex-1 text-center md:text-left space-y-3 md:space-y-4">
        ...
        <p className="text-sm font-medium text-muted-foreground leading-relaxed max-w-lg mx-auto md:mx-0">
          ...
          Submit a request and we'll build a custom quote.
        </p>
      ...
      <div className="shrink-0 flex flex-col gap-3 w-full sm:w-auto">
        <Button
          asChild size="lg"
          className="h-12 md:h-14 px-6 md:px-10 rounded-[var(--radius)] text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-primary/20 group w-full sm:w-auto"
        >
    ...
    <div className="fixed bottom-6 right-6 z-[200] flex flex-col items-end gap-2">
            ...
            {(Object.keys(BOOKING_THEMES) as BookingTheme[]).map(t => (
              <button
                ...
                className={cn(
                  ...
                  t === theme ? 'bg-slate-900 text-white' : 'hover:bg-slate-50 text-slate-700'
                )}
              >
                <div
                  className="w-3 h-3 rounded-full shrink-0"
                  style={{ background: `hsl(${BOOKING_THEMES[t].vars['--primary']})` }}
                />
                <span className="text-[10px] font-black uppercase tracking-widest">
                  {BOOKING_THEMES[t].label}
                </span>
              </button>
            ))}
          </motion.div>
      ...
      >
        <Palette
          ...
          style={{ color: `hsl(${BOOKING_THEMES[theme].vars['--primary']})` }}
        />
...
// Editorial / Bold Studio — centered
const SplashCentered = ({
  tenant, tenantId, theme, onEnter,
}: {
  tenant: any; tenantId: string; theme: BookingTheme; onEnter: () => void;
}) => {
    ...
    >
      <div className="absolute inset-0 z-0">
        <Image
          ...
          alt="Studio" fill className="object-cover opacity-20" priority
        />
      ...
      >
        ...
        <div className="mt-6 sm:mt-8 flex flex-col gap-3 w-full max-w-xs mx-auto">
          <Button
            size="lg" onClick={onEnter}
            className="h-12 sm:h-14 rounded-[var(--radius)] text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-primary/30 group w-full"
          >
            View Service Menu
          ...
          <Button
            ...
            className="h-12 sm:h-14 rounded-[var(--radius)] text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] border-2 bg-background/50 backdrop-blur-sm w-full"
          >
            <Link href={`/kiosk/${tenantId}`}>
              <Users className="mr-2 h-4 w-4" />Join Queue
            </Link>
          </Button>
        </div>
        <motion.div
...
// Soft Spa — split (text left, image right)
const SplashSplit = ({
  tenant, tenantId, onEnter,
}: {
  tenant: any; tenantId: string; onEnter: () => void;
}) => (
  <motion.div
    initial={{ opacity: 1 }} exit={{ opacity: 0 }}
  ...
  >
    ...
    >
      <div className="max-w-sm space-y-8">
        ...
        <div className="space-y-3 w-full">
          <Button
            size="lg" onClick={onEnter}
            className="h-14 w-full rounded-full text-[11px] font-black uppercase tracking-widest shadow-xl shadow-primary/20 group"
          >
            View Services
          ...
          <Button
            size="lg" variant="outline" asChild
            className="h-12 w-full rounded-full text-[10px] font-black uppercase tracking-widest border-2 border-primary/20"
          >
        ...
        <Link
          href={`/inquiry/${tenantId}`}
          className="inline-flex items-center gap-2 text-[9px] font-bold uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors"
        >
          <FileText className="w-3.5 h-3.5" />Request a custom event quote
          <ArrowRight className="w-3 h-3" />
        </Link>
      </div>
    </motion.div>
    ...
    >
      <Image
        src={tenant?.bookingPageSettings?.heroImageUrl || 'https://images.unsplash.com/photo-1604654894610-df63bc536371?q=80&w=1974&auto=format&fit=crop'}
...
// Dark Glam — dramatic full-screen dark with gold
const SplashDramatic = ({
  tenant, tenantId, onEnter,
}: {
  tenant: any; tenantId: string; onEnter: () => void;
}) => (
  ...
  >
    ...
    >
      ...
      >
        <Button
          size="lg" onClick={onEnter}
          className="h-12 sm:h-14 flex-1 text-[10px] font-black uppercase tracking-[0.3em] rounded-none border"
          style={{
            background: 'hsl(44 72% 52% / 0.1)',
            borderColor: 'hsl(44 72% 52% / 0.3)',
            color: 'hsl(44 72% 52%)',
          }}
        >
          Enter Studio
        ...
        <Button
          size="lg" variant="outline" asChild
          className="h-12 sm:h-14 flex-1 text-[10px] font-black uppercase tracking-[0.3em] rounded-none bg-transparent"
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4, duration: 0.6 }}>
        <Link
          href={`/inquiry/${tenantId}`}
          className="inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] hover:opacity-80 transition-opacity"
...
// Minimal Clean — no image, pure type
const SplashMinimal = ({
  tenant, tenantId, onEnter,
}: {
  tenant: any; tenantId: string; onEnter: () => void;
  ...
  >
    <div
      ...
      style={{
        backgroundImage: 'repeating-linear-gradient(0deg,currentColor 0px,currentColor 1px,transparent 1px,transparent 40px),repeating-linear-gradient(90deg,currentColor 0px,currentColor 1px,transparent 1px,transparent 40px)',
      }}
    />
    <motion.div
    ...
    >
      ...
      <div className="flex flex-col gap-3 w-full max-w-xs">
        <Button
          size="lg" onClick={onEnter}
          className="h-12 w-full rounded-sm text-[10px] font-black uppercase tracking-[0.2em]"
        >
          Book an Appointment
        </Button>
        <Button
          size="lg" variant="outline" asChild
          className="h-10 w-full rounded-sm text-[9px] font-bold uppercase tracking-[0.2em] border border-border shadow-none"
        >
      ...
      <Link
        href={`/inquiry/${tenantId}`}
        className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors border-b border-border pb-0.5"
      >
...
const StickyNav = ({
  ...
  <div className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-xl px-4 md:px-8 py-3 md:py-4 flex items-center justify-between gap-3">
    <div className="flex items-center gap-2 min-w-0">
      ...
      <span className="font-black uppercase tracking-tighter text-base sm:text-xl truncate">
        {tenant?.name}
      </span>
    </div>
...
export default function BookingPage() {
  ...
  const appointments = useMemo(() =>
    (appointmentsFromDB || []).map(a => ({
      ...a,
      startTime: safeDate(a.startTime),
      endTime:   safeDate(a.endTime),
    })),
    [appointmentsFromDB]
  );
  ...
  const events = useMemo(() =>
    (eventsFromDB || []).map(e => ({
      ...e,
      startTime: safeDate(e.startTime),
      endTime:   safeDate(e.endTime),
    })),
    [eventsFromDB]
  );

  // ── Active theme ──────────────────────────────────────────────────────────
  const activeTheme: BookingTheme =
    previewTheme ??
  ...
  // ── Inject CSS vars onto documentElement so BookingSheet dialogs get themed
  useEffect(() => {
    if (!tenant) return;
    const config = BOOKING_THEMES[activeTheme];
    const root   = document.documentElement;

    // Color + radius variables (shadcn tokens)
    Object.entries(config.vars).forEach(([key, value]) => {
      root.style.setProperty(key, value);
    });

    // Font variables (read by BookingSheet via var(--booking-heading-font))
    root.style.setProperty('--booking-heading-font', config.headingFont);
    root.style.setProperty('--booking-body-font',    config.bodyFont);
  ...
  const handleConfirmBooking = async (
    ...
    try {
      ...
      const qs = await getDocs(
        query(clientsRef, where('email', '==', formData.clientEmail.toLowerCase()))
      );
      ...
      if (qs.empty) {
        ...
        clientId   = qs.docs[0].id;
        clientName = qs.docs[0].data().name;
      }
      ...
      const newApt       = {
        ...appointmentDetails,
        id: aptRef.id, tenantId, clientId, clientName,
      ...
      batch.set(
        doc(firestore, 'appointmentCheckIns', checkInToken),
        sanitizeForFirestore(newApt)
      );

      signedForms.forEach(f => {
        const r = doc(collection(firestore, `tenants/${tenantId}/clients/${clientId}/signedConsents`));
        batch.set(r, sanitizeForFirestore({
          ...f, id: r.id, clientId, signedAt: new Date().toISOString(),
        }));
      });

      if (newApt.staffId) {
        const nr = doc(collection(firestore, `tenants/${tenantId}/notifications`));
        batch.set(nr, sanitizeForFirestore({
          id:        nanoid(),
          userId:    newApt.staffId,
          type:      'new_appointment',
          message:   `New booking: ${formData.clientName} for ${selectedService?.name}`,
          link:      '/planner',
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center gap-4 bg-background">
        ...
        <p className="text-xs sm:text-sm font-black uppercase tracking-[0.2em] text-muted-foreground animate-pulse">
          Initializing Studio...
        </p>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {/* Theme switcher — remove or gate behind admin flag in production */}
