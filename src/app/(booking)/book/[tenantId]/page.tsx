...
const DS: StyleConfig = {
  accentColor:'#8b6914', bgColor:'#f8f4ef', headingFont:'cormorant',
  bodyFont:'space', borderRadius:8, buttonStyle:'filled', density:'balanced',
};
...
// ─── Permissive config guard ───────────────────────────────────────────────────
// Just verify it has a non-empty sections array — trust whatever the builder saved.
function isBuilderConfig(pc: any): boolean {
  return Array.isArray(pc?.sections) && pc.sections.length > 0;
}

// ─── Fallback defaults (shown only when there is no saved config at all) ───────
...
function ReviewsSection({ config, style, isPreview, sectionId, onFieldTap }: SectionProps) {
  ...
  const reviews = [1,2,3,4,5,6].map(n=>({
    ...
    text: config[`rev${n}Text`]||['Absolutely incredible experience. The attention to detail is unmatched.','I\'ve been coming here over a year and every visit exceeds expectations.','The atmosphere is luxurious yet so welcoming. I always feel like a VIP.','Cannot recommend enough.','A gem of a studio. Every appointment is a joy.','So glad I found this place.'][n-1]||'',
...
function BookingPageContent({ tenantId }: { tenantId: string }) {
  // Is this page inside the page builder iframe?
  ...
  // ── Config ───────────────────────────────────────────────────────────────
  // savedConfig  → what's persisted in Firestore; the only source on the public page
  // liveConfig   → real-time updates from the builder iframe; only used when isPreview
  ...
  // ── Loading gates ────────────────────────────────────────────────────────
  const [configReady,setConfigReady] = useState(false);   // Firestore fetch done
  const [liveReady,setLiveReady]     = useState(false);   // Builder sent first message
  ...
  // ── Phase 1: load tenant doc + saved page config ─────────────────────────
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled && tSnap.exists()) {
          ...
          // Debug: visible in browser console — remove after confirming config loads
          console.log('[booking] tenantId:', tenantId);
          console.log('[booking] cfPageConfig keys:', pc ? Object.keys(pc) : 'NOT FOUND');
          console.log('[booking] sections count:', pc?.sections?.length ?? 0);

          if (isBuilderConfig(pc)) {
            setSavedConfig(pc as PageBuilderConfig);
            console.log('[booking] ✓ savedConfig loaded');
          } else {
            console.warn('[booking] ✗ isBuilderConfig rejected — falling back to defaults');
          }
        } else if (!cancelled) {
          console.warn('[booking] tenant doc not found for id:', tenantId);
        }
      } catch (e) {
  ...
  // ── Preview: receive live config from builder ─────────────────────────────
  useEffect(() => {
    if (!isPreview) return;
  ...
  // ── Preview: signal builder that iframe is ready ──────────────────────────
  useEffect(() => {
    if (!isPreview) return;
  ...
  // ── Resolve config ────────────────────────────────────────────────────────
  // PUBLIC page  → always use savedConfig (liveConfig is never set)
  // PREVIEW iframe → liveConfig takes precedence so builder changes are instant
  ...
  // ── Side-effects that depend on resolved config ───────────────────────────
  ...
  // ── Loading spinner ───────────────────────────────────────────────────────
  // Public: unblocks as soon as Firestore fetch is done.
  // Preview: also waits for the builder to send its first live config
  //          so we don't flash the saved state before the builder syncs.
