...
export default function WalkInKioskPage() {
  ...
  const freeProviderSet = React.useRef<Set<string>>(new Set()).current;
  const holdingOutMap = React.useRef<Map<string, string>>(new Map());
  ...
  const refreshFloor = useCallback(async () => {
    ...
    try {
      ...
      if (d?.ok) {
        ...
        const mL = Number(d.minutesUntilClose), sM = Number(d.softCloseMinutes)||60, ct = d.closingTime||'';
        setClosingWarning((mL>0&&mL<=sM&&ct) ? `Heads up — we close at ${ct}. Waits may be longer than usual.` : null);
        const nowFree = new Set<string>(
          (d.floor || []).filter((f: any) => f.accepting !== false && f.busy !== true)
            .map((f: any) => String(f.staffId || f.id || '')).filter(Boolean),
        );
        holdingOutMap.current.forEach((guestName, staffId) => {
          if (freeProviderSet.has(staffId) && !nowFree.has(staffId)) {
            toast({ variant: 'destructive', title: 'Provider unavailable',
              description: `${guestName} is holding out for a provider who just went on break or clocked out.` });
          }
        });
        freeProviderSet.clear();
        nowFree.forEach(id => freeProviderSet.add(id));
      }
    } catch { /* keep the last known status rather than flashing a closed sign */ }
  }, [tenantId, toast, freeProviderSet, holdingOutMap]);
  ...
  const submitPhone = async () => {
    ...
    try {
      ...
      const availPaths = (d?.paths || []).filter((p: string) => p !== 'in-line');
      if (availPaths.length > 1) { setLookup(d); setStep('choosepath'); return; }
  ...
  const join = async (acknowledgePatchTest: boolean) => {
    ...
    try {
      ...
      if (d?.closed || d?.capacityReason === 'hard_close') {
        setFailMessage(d?.error || 'Walk-ins have just been switched off — please see the front desk.');
        setStep('full');
      } else if (d?.capacityReason === 'full') {
        setFailMessage(d?.error || 'The queue is full right now.');
        setStep('full');
      } else {
        setTroubleNote(d?.error || '');
        setStep('trouble');
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-b from-rose-50 via-white to-white text-slate-900 flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-5 sm:px-6 pb-10 w-full max-w-lg mx-auto">
        ...
        {!kioskOff && step === 'welcome' && (
          ...
          >
            ...
            <div className="space-y-3">
              ...
              {closingWarning && (
                <div className="mt-3 flex items-center gap-2.5 bg-amber-50 border-2 border-amber-200 rounded-2xl px-4 py-3 text-left">
                  <AlertTriangle className="w-5 h-5 text-amber-500 shrink-0" />
                  <p className="text-sm text-amber-700 font-medium">{closingWarning}</p>
                </div>
              )}
            </div>
            {floor && floor.open && (
        ...
        {!kioskOff && step === 'choosepath' && lookup && (
          <div className="w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500">What brings you in today?</p>
            </div>
            <div className="space-y-3">
              {lookup.appointmentToday && (
                <button onClick={() => setStep('checkin')}
                  className="w-full h-14 min-h-[44px] rounded-2xl border-2 border-slate-200 bg-white text-base font-semibold flex items-center gap-3 px-5 active:scale-[0.99] transition-all">
                  <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" />
                  Check in for my appointment
                </button>
              )}
              {lookup.boothToday && (
                <button onClick={() => setStep('booth')}
                  className="w-full h-14 min-h-[44px] rounded-2xl border-2 border-slate-200 bg-white text-base font-semibold flex items-center gap-3 px-5 active:scale-[0.99] transition-all">
                  <Ticket className="w-5 h-5 text-indigo-500 shrink-0" />
                  Start my booth rental
                </button>
              )}
              {(lookup.paths || []).includes('walk-in') && (
                <button onClick={() => setStep(lookup.usual?.serviceId ? 'recognize' : 'service')}
                  className="w-full h-14 min-h-[44px] rounded-2xl border-2 border-slate-200 bg-white text-base font-semibold flex items-center gap-3 px-5 active:scale-[0.99] transition-all">
            ...
            <button onClick={reset} className="w-full min-h-[44px] text-sm text-slate-400 py-1">Not you? Start over</button>
          </div>
        )}
