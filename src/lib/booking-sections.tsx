...
function PoliciesSection({ config, style, isPreview, sectionId, onFieldTap }: SectionProps) {
        ...
        {policyItems.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-6">
            {policyItems.map((p: any, i: number) => (
              <div key={p.id || i} className="p-7 bg-white space-y-3 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                style={{ borderRadius: br(style, 1.5), border: `2px solid ${ac(style)}22` }}>
                <div className="flex items-center gap-2.5">
                  <span className="text-xl">{ie[p.icon] || '🛡'}</span>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-16">
        ...
        {policyItems.length > 0 ? (
          <div className="max-w-2xl mx-auto space-y-5">
            {policyItems.map((p: any, i: number) => (
              <div key={p.id || i} className="flex items-start gap-4 py-4 border-b"
                style={{ borderColor: ac(style) + '18' }}>
                <span className="text-xl shrink-0 mt-0.5">{ie[p.icon] || '🛡'}</span>
                <div>
                  <p className="text-sm font-black uppercase tracking-tight text-slate-900 mb-1"
                    style={{ fontFamily: bf(style) }}>{p.title}</p>
                  <p className="text-sm text-slate-500 leading-relaxed" style={{ fontFamily: bf(style) }}>{p.body}</p>
                </div>
              </div>
            ))}
  ...
  // ── timeline ───────────────────────────────────────────────────────────────
  // Vertical timeline with alternating left/right cards, connected by a line
        ...
        {policyItems.length > 0 ? (
          <div className="relative">
            ...
            <div className="absolute left-1/2 top-0 bottom-0 w-px hidden md:block"
              style={{ background: `linear-gradient(to bottom, transparent, ${ac(style)}30, ${ac(style)}30, transparent)`, transform: 'translateX(-50%)' }} />
            <div className="space-y-8 md:space-y-0">
              {policyItems.map((p: any, i: number) => {
                const isLeft = i % 2 === 0;
                return (
                  <div key={p.id || i}
                    className={`relative md:grid md:grid-cols-2 md:gap-10 items-center mb-8`}
                    style={{ animation: `cf-fade-up 0.6s ${i * 0.1}s both` }}>
                    {/* Left side */}
                    <div className={isLeft ? 'md:text-right' : 'md:order-2 md:text-left'}>
                      <div className={`bg-white p-6 shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ${isLeft ? 'md:ml-auto' : ''}`}
                        style={{ borderRadius: br(style, 2), border: `2px solid ${ac(style)}18`, maxWidth: '400px' }}>
                        <div className={`flex items-center gap-3 mb-3 ${isLeft ? 'md:flex-row-reverse' : ''}`}>
                          ...
                          <p className="text-[11px] font-black uppercase tracking-widest"
                            style={{ color: ac(style) }}>{p.title}</p>
                        </div>
                        <p className="text-sm text-slate-500 leading-relaxed" style={{ fontFamily: bf(style) }}>{p.body}</p>
                      </div>
                    </div>
                    {/* Center dot */}
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:flex w-10 h-10 rounded-full items-center justify-center z-10"
                      style={{ background: 'white', border: `3px solid ${ac(style)}`, boxShadow: `0 0 0 4px ${ac(style)}18` }}>
                      <span className="text-[10px] font-black" style={{ color: ac(style) }}>{String(i + 1).padStart(2, '0')}</span>
                    </div>
                    {/* Right placeholder (empty for alternating) */}
                    {!isLeft && <div className="md:order-1" />}
                    {isLeft && <div />}
                  </div>
                );
              })}
  ...
  // ── accordion ──────────────────────────────────────────────────────────────
  // Bold numbered accordion with smooth expand, accent left border
        ...
        {policyItems.length > 0 ? (
          <div className="space-y-2">
            {policyItems.map((p: any, i: number) => {
              ...
              return (
                <div key={p.id || i}
                  className="overflow-hidden bg-white transition-all duration-300 hover:shadow-md"
                  style={{
                    borderRadius: br(style, 1.5),
                    border: `2px solid ${isOpen ? ac(style) + '60' : ac(style) + '18'}`,
                    borderLeft: `4px solid ${isOpen ? ac(style) : ac(style) + '30'}`,
                  }}>
                  <button
                    className="w-full flex items-center gap-5 px-6 py-5 text-left hover:bg-slate-50/80 transition-colors"
                    onClick={() => setOpenIdx(isOpen ? null : i)}>
                    <span className="text-3xl font-light shrink-0 tabular-nums w-10 text-right"
                      style={{ fontFamily: hf(style), color: isOpen ? ac(style) : ac(style) + '35' }}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <span className="text-xl shrink-0">{ie[p.icon] || '🛡'}</span>
                    <span className="flex-1 font-black text-sm uppercase tracking-tight text-slate-900 pr-4"
                      style={{ fontFamily: bf(style) }}>{p.title}</span>
                    <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all duration-300"
                      style={{ background: isOpen ? ac(style) : ac(style) + '10' }}>
                      <ChevronDown className="w-3.5 h-3.5 transition-transform duration-300"
                        style={{ color: isOpen ? 'white' : ac(style), transform: isOpen ? 'rotate(180deg)' : 'none' }} />
                    </div>
                  </button>
                  <div style={{
                    ...
                    <p className="px-6 pb-6 text-sm text-slate-500 leading-relaxed pl-[calc(2.5rem+1.25rem+1.25rem+1.5rem)]"
  ...
  // ── editorial ──────────────────────────────────────────────────────────────
  // Magazine-style with large index numbers, ruled lines, typographic hierarchy
  if (layout === 'editorial') return (
    <section className={py(style)} style={{ background: style.bgColor }}>
      <div className="max-w-5xl mx-auto px-6 md:px-16">
        <div className="flex items-end justify-between mb-14 pb-6 border-b-2" style={{ borderColor: ac(style) + '20' }}>
          <FieldTap sectionId={sectionId} fieldKey="heading" isPreview={isPreview} onFieldTap={onFieldTap}
            as="h2" className="text-4xl md:text-6xl font-light leading-none"
            style={{ fontFamily: hf(style), color: '#0f172a' }}>
            {config.heading || 'Our Policies'}
          </FieldTap>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-2 hidden md:block"
            style={{ color: ac(style) + '60' }}>
            {policyItems.length} items
          </p>
        </div>
        {policyItems.length > 0 ? (
          <div className="space-y-0">
            {policyItems.map((p: any, i: number) => (
              <div key={p.id || i}
                className="group grid md:grid-cols-[80px_1fr_2fr] gap-0 items-start py-8 border-b hover:bg-slate-50/50 transition-colors px-2 -mx-2 cursor-default"
                style={{ borderColor: ac(style) + '12' }}>
                {/* Index */}
                <div className="hidden md:flex items-start justify-end pr-6 pt-1">
                  <span className="text-3xl font-light tabular-nums"
                    style={{ fontFamily: hf(style), color: ac(style) + '30', transition: 'color 0.2s' }}>
                    {String(i + 1).padStart(2, '0')}
                  </span>
                </div>
                {/* Title + icon */}
                <div className="flex items-start gap-3 md:pr-10 mb-3 md:mb-0">
                  <span className="text-lg mt-0.5">{ie[p.icon] || '🛡'}</span>
                  <div>
                    <p className="text-sm font-black uppercase tracking-tight text-slate-900 leading-snug"
                      style={{ fontFamily: bf(style) }}>{p.title}</p>
                    <div className="w-8 h-0.5 mt-2 transition-all duration-500 group-hover:w-14"
  ...
  // ── dark ───────────────────────────────────────────────────────────────────
  // Dark luxury background, glowing accent cards, minimal high-contrast type
  if (layout === 'dark') return (
    <section className={py(style)} style={{ background: '#0c0c0e', position: 'relative', overflow: 'hidden' }}>
      {/* Ambient glow */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${ac(style)}15 0%, transparent 65%)` }} />
      <div className="relative max-w-5xl mx-auto px-6 md:px-16">
        <div className="text-center mb-14 space-y-3">
          <FieldTap sectionId={sectionId} fieldKey="heading" isPreview={isPreview} onFieldTap={onFieldTap}
            as="h2" className="text-4xl md:text-5xl font-light text-white"
            style={{ fontFamily: hf(style) }}>
            {config.heading || 'Our Policies'}
          </FieldTap>
          {config.subheading && (
            <p className="text-base max-w-xl mx-auto" style={{ fontFamily: bf(style), color: 'rgba(255,255,255,0.45)' }}>
              {config.subheading}
            </p>
          )}
        </div>
        {policyItems.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-4">
            {policyItems.map((p: any, i: number) => (
              <div key={p.id || i}
                className="group relative p-7 space-y-4 hover:-translate-y-1 transition-all duration-400 cursor-default"
                style={{
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                  style={{ borderRadius: br(style, 2), background: `radial-gradient(ellipse at 50% 0%, ${ac(style)}18 0%, transparent 70%)` }} />
                {/* Top accent line */}
                ...
                <div className="relative flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-lg"
                    style={{ background: ac(style) + '18', border: `1px solid ${ac(style)}30` }}>
                    {ie[p.icon] || '🛡'}
                  </div>
                  <p className="text-[10px] font-black uppercase tracking-widest"
                    style={{ color: ac(style) }}>{p.title}</p>
                </div>
                <p className="relative text-sm leading-relaxed" style={{ fontFamily: bf(style), color: 'rgba(255,255,255,0.55)' }}>
                  {p.body}
                </p>
                <div className="absolute bottom-4 right-5 text-[9px] font-black tabular-nums"
                  style={{ color: ac(style) + '35' }}>
                  {String(i + 1).padStart(2, '0')}
                </div>
              </div>
            ))}
          ...
          <p className="text-center text-[11px] font-black uppercase tracking-widest py-12"
    ...
    <section className={py(style)} style={{ background: style.bgColor }}>
      <div className="max-w-5xl mx-auto px-6 md:px-16">
        ...
        {policyItems.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-6">
            {policyItems.map((p: any, i: number) => (
              <div key={p.id || i} className="p-7 bg-white space-y-3"
                style={{ borderRadius: br(style, 1.5), border: `2px solid ${ac(style)}22` }}>
                <div className="flex items-center gap-2.5">
                  <span className="text-xl">{ie[p.icon] || '🛡'}</span>
                  <p className="text-[11px] font-black uppercase tracking-widest" style={{ color: ac(style) }}>{p.title}</p>
