 ...
 * stored on the device (localStorage — survives navigation to /messages,
 * which is a separate route outside the portal's component tree). The
 ...
 * path unchanged.
 *
 * TRUST MODEL — being honest: localStorage identity is client-side trust,
...
export function resolveActiveStaffId(authUid?: string | null): string | null {
  if (typeof window !== 'undefined') {
    const stored = window.localStorage.getItem(ACTIVE_STAFF_KEY);
...
export function setActiveStaffId(staffId: string) {
  if (typeof window !== 'undefined') window.localStorage.setItem(ACTIVE_STAFF_KEY, staffId);
}

export function clearActiveStaffId() {
  if (typeof window !== 'undefined') window.localStorage.removeItem(ACTIVE_STAFF_KEY);
}
