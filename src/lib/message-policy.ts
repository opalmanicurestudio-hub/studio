...
export const MESSAGE_KINDS: MessageKindDef[] = [
  ...
  {
    id: 'renter_message', group: 'Renters', label: 'Message from the studio',
    when: 'You write to a renter from their card, reply to a concern, or send an update on an interruption.',
    channels: ['email', 'sms'], canDisable: false,
    tokens: ['{{renter_first}}', '{{studio}}', '{{link}}'],
    requiredTokens: ['{{link}}'],
    defaultSubject: 'A message from {{studio}}',
    defaultBody: '{{renter_first}}, you have a message from {{studio}}. Read and reply in your portal so we both keep a record.\n\n{{link}}',
    timing: 'immediate',
  },
  {
    id: 'renter_broadcast', group: 'Renters', label: 'Note to every renter',
    when: 'You send one message to the whole floor, or the renters you picked.',
    channels: ['email', 'sms'], canDisable: true,
    tokens: ['{{renter_first}}', '{{studio}}', '{{link}}'],
    requiredTokens: ['{{link}}'],
    defaultSubject: 'A note to everyone from {{studio}}',
    defaultBody: '{{renter_first}}, a note from {{studio}} to every renter.\n\n{{link}}',
    timing: 'immediate',
  },
  {
    id: 'grievance_received', group: 'Renters', label: 'Concern — receipt',
    when: 'A renter raises a concern from their portal. The receipt carries their reference number.',
    channels: ['email'], canDisable: false,
    tokens: ['{{renter_first}}', '{{studio}}'],
    requiredTokens: [],
    defaultSubject: 'We have your concern',
    defaultBody: '{{renter_first}}, your concern is on file. Keep the reference and quote it if you follow up; you will see its status in your portal.',
    timing: 'immediate',
  },
  {
