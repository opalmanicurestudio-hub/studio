'use client';
/**
 * EventModeScreen — replaces the bare emoji grid.
 *
 * New flow:
 *   1. Table prompt  → guest enters table number once (persisted in sessionStorage)
 *   2. Name lookup   → we check eventGuests for their pre-order → greet by first name
...
import { Check, ChevronRight, Loader, CheckCircle2, ArrowLeft } from 'lucide-react';
...
// ─── Types ────────────────────────────────────────────────────────────────────
type KioskTheme = any; // uses parent T interface
...
const AmbientBg = ({ hex }: { hex?: string }) => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    ...
    <div className="absolute -top-1/3 -left-1/4 w-2/3 h-2/3 rounded-full blur-[100px] opacity-15 transition-all duration-1000"
      style={{ backgroundColor: hex || '#c9a96e' }} />
    <div className="absolute -bottom-1/4 -right-1/4 w-1/2 h-1/2 rounded-full blur-[80px] opacity-10 transition-all duration-1000"
      style={{ backgroundColor: hex || '#c9a96e' }} />
  </div>
);

// ─── Step: Table number entry ─────────────────────────────────────────────────
const TableStep = ({ onConfirm, logoUrl, tenantName, eventName, hex }: {
  onConfirm: (table: string) => void;
  logoUrl?: string; tenantName?: string; eventName?: string; hex?: string;
}) => {
  const [table, setTable] = useState('');
  return (
    <motion.div key="table-step"
      ...
      <div className="w-full max-w-sm space-y-10 text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          className="space-y-4">
          {/* Large table number input */}
          <input
            type="text" inputMode="numeric" value={table}
            ...
            autoFocus
          />
          <p className="text-[9px] text-white/20 uppercase tracking-[0.25em] font-['DM_Sans']">Table Number</p>
        </motion.div>
...
// ─── Request button ───────────────────────────────────────────────────────────
const RequestButton = ({ req, status, onTap, hex }: {
  req: FloorRequest;
    ...
    <motion.button
      ...
      <div className="text-center">
        ...
        <p className={cn(
          ...
          {status === 'acknowledged' ? 'On the way ✓' :
           isDone ? 'Requested' :
           req.description}
        </p>
      </div>
...
interface EventModeScreenProps {
  event: any;
  tenant: any;
  t: any; // KioskTheme tokens — kept for compatibility but we override with dark aesthetic
...
export function EventModeScreen({
  event, tenant, onFloorRequest, onExit, firestore, tenantId,
  ...
  // ── Table session ─────────────────────────────────────────────────────────
  // Persisted in sessionStorage so it survives across request taps
  ...
  const lookupGuest = async (table: string) => {
    ...
    try {
      ...
      if (!snap.empty) {
        // Multiple guests at table — greet the first checked-in one
        // In practice staff check in guests so this is the right person
  ...
  const handleRequest = async (req: FloorRequest) => {
    ...
    // Deduplication: check for recent identical request from this table
    if (firestore && tenantId) {
      try {
        ...
        // Filter client-side for recency (Firestore doesn't support inequality on different fields)
        ...
        if (recentDup) {
          ...
          toast({
            ...
            description: ackd ? 'Your request has been acknowledged.' : 'Staff have been notified — they\'ll be with you shortly.',
          });
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-['Cormorant_Garamond'] text-sm text-white/50 italic">{eventName}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-[7px] font-bold uppercase tracking-[0.3em] text-white/20">Table</p>
            <p className="font-['Cormorant_Garamond'] text-2xl text-white leading-none">{tableNumber}</p>
          </div>
        </div>
