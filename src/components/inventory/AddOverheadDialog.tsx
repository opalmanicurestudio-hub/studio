  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-6 text-left">
        ...
        <div className="space-y-2">
          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Asset Description</Label>
          <Textarea placeholder="Internal details regarding this overhead supply..." {...register('description')} className="rounded-2xl border-2 bg-muted/5 min-h-[100px] p-4 font-medium" />
        </div>
      </div>
    </div>
  );
...
const Step2 = () => {
  const { control, register, watch } = useFormContext<OverheadFormData>();
        ...
        <div className="space-y-6">
          ...
          {costingMethod === 'size' ? (
            <div className="grid grid-cols-2 gap-4 text-left">
              ...
              <div className="space-y-1.5">
                <Label className="text-[9px] font-black uppercase text-muted-foreground ml-1">Unit</Label>
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start text-left">
        <div className="space-y-6">
          ...
          <div className="space-y-1.5">
            <Label className="text-[9px] font-black uppercase text-muted-foreground ml-1">Source / Supplier</Label>
            <Input placeholder="e.g., Uline" {...register('supplier')} className="h-11 rounded-xl border-2 font-bold" />
          </div>
        </div>
        <div className="space-y-6">
          <div className="space-y-1.5">
...
export const AddOverheadDialog = ({
  ...
  const methods = useForm<OverheadFormData>({
    ...
    defaultValues: { costingMethod: 'uses', initialStock: 1 },
  });
  const { handleSubmit, trigger, reset } = methods;

  useEffect(() => {
    if (open) { reset({ costingMethod: 'uses', initialStock: 1, purchaseDate: new Date() }); setStep(1); }
  }, [open, reset]);
