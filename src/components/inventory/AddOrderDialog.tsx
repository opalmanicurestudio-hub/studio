...
import { CalendarIcon, PlusCircle, Trash2, DollarSign, Sparkles, Truck, PackageOpen, Calculator, Landmark, ShieldCheck, ArrowRight, Check, Search, Plus, Info } from 'lucide-react';
    ...
    const handleAddProducts = (selectedProducts: InventoryItem[]) => {
        ...
        setItems(prev => {
          ...
          newItems.forEach(newItem => {
            if (!combined.find(item => item.productId === newItem.productId)) combined.push(newItem);
          });
          return combined;
        });
    };
                                ...
                                {items.map(item => (
                                    <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl border-2 bg-white shadow-sm group" key={item.productId}>
                                        ...
                                        <div className="flex items-center gap-3 justify-between w-full sm:w-auto">
                                            ...
                                            <Button variant="ghost" size="icon" className="h-10 w-10 text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleRemoveItem(item.productId)}><Trash2 className="w-4 h-4"/></Button>
                                        </div>
                                    </div>
                                ))}
                            </div>
        ...
        <DialogContainer open={open} onOpenChange={onOpenChange}>
            <ContentComponent side={isMobile ? "bottom" : "right"} className={cn("p-0 border-none bg-background flex flex-col shadow-3xl overflow-hidden", isMobile ? "h-[92dvh] rounded-t-[3rem]" : "sm:max-w-4xl max-h-[90dvh]")}>
                <DialogHeader className={cn("flex-shrink-0 text-left border-b bg-muted/5", isMobile ? "p-6" : "p-8 pb-6")}>
                    ...
                    <DialogTitle className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-slate-900 leading-none">Register Order Protocol</DialogTitle>
                    <DialogDescription className="text-[10px] font-bold uppercase tracking-widest opacity-60 mt-1">Log supplier shipments into the studio ledger.</DialogDescription>
                ...
                <DialogFooter className={cn("border-t bg-background flex-shrink-0 shadow-2xl", isMobile ? "p-4" : "p-6 sm:p-8 pt-4")}>
                    <div className='flex w-full gap-4'>
                        ...
                        <div className={cn("flex gap-3", step === 1 ? "w-full" : "flex-[2.5]")}>
                            ...
                            {step < totalSteps ? (
                                <Button onClick={() => setStep(step + 1)} disabled={items.length === 0} className="flex-[1.5] h-12 md:h-16 font-black uppercase tracking-widest text-[10px] md:text-xl rounded-[2rem] shadow-2xl shadow-primary/30 group">Continue <ArrowRight className="ml-3 w-4 h-4 md:w-8 md:h-8 transition-transform group-hover:translate-x-1" /></Button>
                            ) : (
                                <Button onClick={handleSave} className="flex-[1.5] h-12 md:h-16 font-black uppercase tracking-widest text-[10px] md:text-xl rounded-[2rem] shadow-2xl shadow-primary/30">Save Order</Button>
                            )}
                        </div>
                    </div>
                </DialogFooter>
