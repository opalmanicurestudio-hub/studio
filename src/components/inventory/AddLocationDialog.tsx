...
export const AddLocationDialog = ({
  ...
  const formBody = (
    <FormProvider {...methods}>
        <form id="add-location-strategic-form" onSubmit={handleSubmit(handleSave)} className="flex flex-col flex-1 min-h-0">
            <DialogHeader className={cn("flex-shrink-0 text-left border-b bg-muted/5", isMobile ? "p-6" : "p-8 pb-6")}>
                ...
                <DialogTitle className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-slate-900 leading-none">Register New Zone</DialogTitle>
                <DialogDescription className="text-[10px] font-bold uppercase tracking-widest opacity-60 mt-1">Establish a physical storage unit within the studio manifest.</DialogDescription>
            </DialogHeader>
                ...
                <div className={cn("pb-32 space-y-12", isMobile ? "p-6" : "p-8")}>
                    <div className="space-y-10">
                        ...
                        <div className="space-y-6 text-left">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="location-type" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Unit Classification</Label>
                                {isAddingType ? (
                                            ...
                                            <RadioGroup value={newTypeIcon} onValueChange={setNewTypeIcon} className="grid grid-cols-4 gap-2">
                                                {Object.entries(iconMap).map(([iconKey, { component: Icon, label }]) => (
                                                    <div key={iconKey}>
                                                        ...
                                                        <Label htmlFor={`new-type-${iconKey}`} className={cn("flex flex-col items-center justify-center rounded-xl border-2 border-muted bg-white p-3 text-[10px] font-black uppercase tracking-tighter hover:bg-primary/[0.03] peer-data-[state=checked]:border-primary peer-data-[state=checked]:text-primary transition-all cursor-pointer h-16 shadow-sm")}>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Select onValueChange={field.onChange} value={field.value}>
                                                    <SelectTrigger id="location-type" className="h-14 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest shadow-inner bg-muted/5 flex-1">
                    ...
                    <div className="space-y-10 pt-10 border-t border-dashed">
                        ...
                        <div className="space-y-8 text-left">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="customNeeds" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Ancillary Protocol Requirements</Label>
                                <Input id="customNeeds" placeholder="e.g., MUST BE STORED UPRIGHT" {...control.register('customNeeds')} className="h-12 rounded-xl border-2 font-black uppercase text-xs shadow-inner bg-muted/5" />
                            </div>

                            <div className="space-y-2">
                                <Label htmlFor="location-description" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Operational Description</Label>
                                <Textarea id="location-description" placeholder="Specify what is typically archived in this zone..." {...control.register('description')} className="rounded-2xl border-2 bg-muted/5 min-h-[100px] focus-visible:ring-primary/20" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <DialogFooter className={cn("border-t bg-background flex-shrink-0 shadow-2xl", isMobile ? "p-4" : "p-6 sm:p-8 pt-4")}>
                <div className="grid grid-cols-2 gap-3 w-full">
                    ...
                    <Button type="submit" className="h-12 rounded-[2rem] font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-primary/30 active:scale-95 transition-all group">Establish Zone <ArrowRight className="ml-2 w-4 h-4 transition-transform group-hover:translate-x-1"/></Button>
                </div>
            </DialogFooter>
        </form>
    </FormProvider>
