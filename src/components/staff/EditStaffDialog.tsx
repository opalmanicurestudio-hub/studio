        ...
        <div className="space-y-8">
          ...
          <div className="flex flex-col items-center gap-8 p-6 rounded-[2.5rem] border-2 bg-muted/5 shadow-inner">
            ...
            <div className="flex-1 space-y-4 w-full text-left">
              <div className="space-y-1.5 text-left">
                <Label htmlFor="name-edit" className="text-[9px] uppercase font-black text-muted-foreground tracking-widest ml-1">
                  Legal Name
                </Label>
                <Input
                  id="name-edit"
                  ...
                  {...register('name')}
                  className="h-12 rounded-xl border-2 font-black uppercase tracking-tight text-base shadow-inner bg-white"
                />
                {errors.name && (
                  <p className="text-[9px] font-bold text-destructive uppercase ml-1">{errors.name.message}</p>
                )}
              </div>
              <div className="space-y-1.5 text-left">
                <Label htmlFor="email-edit" className="text-[9px] uppercase font-black text-muted-foreground tracking-widest ml-1">
                  Professional Email
                </Label>
                <Input
                  id="email-edit"
          ...
          <div className="p-6 bg-primary/5 rounded-[2.5rem] border-4 border-primary/10 space-y-4 shadow-inner">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="h-7 text-[9px] font-black uppercase tracking-widest text-primary hover:bg-primary/10"
              >
          ...
          <div className="flex items-center justify-between p-6 border-2 border-dashed rounded-[2rem] bg-muted/5">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <Switch id="public-toggle-edit" checked={field.value} onCheckedChange={field.onChange} className="scale-125" />
              )}
            />
          </div>
        </div>
      ...
      <div className="space-y-10">
        ...
        <div className="space-y-8 text-left">
          ...
          <div className="space-y-2 text-left">
            ...
            <Input
              id="specialties-edit"
              placeholder="e.g., BALAYAGE, PRECISION CUTS"
              {...register('specialties')}
              className="h-12 rounded-xl border-2 font-black uppercase text-xs shadow-inner bg-white"
            />
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-left">
                {selectedServices.map(service => (
                  <div key={service.id} className="flex items-center justify-between p-3 rounded-xl border-2 bg-white shadow-sm group">
                    <span className="text-[10px] font-black uppercase tracking-tight text-slate-900 truncate flex-1 text-left">
                      {service.name}
                    </span>
                    <Button
                      variant="ghost"
              ...
              <div className="grid grid-cols-1 gap-2 text-left">
                {assignedForms.map(form => (
                  <div key={form.id} className="flex items-center justify-between p-3 rounded-xl border-2 bg-white shadow-sm group">
                    <span className="text-[10px] font-black uppercase tracking-tight text-slate-900 truncate text-left">
                      {form.title}
                    </span>
                    <Button
                      variant="ghost"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="commissionRate-edit" type="number" placeholder="40" {...register('commissionRate')} className="h-12 pr-8 rounded-xl border-2 font-black text-lg text-primary shadow-inner" />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="retailCommissionRate-edit" type="number" placeholder="10" {...register('retailCommissionRate')} className="h-12 pr-8 rounded-xl border-2 font-black text-lg text-primary shadow-inner" />
                  <Percent className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-primary opacity-40" />
                </div>
              </div>
            </div>
        ...
        <div className="space-y-10 text-left">
          ...
          <div className="space-y-4 pt-4 border-t border-dashed text-left">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2 opacity-60">
              <ShieldCheck className="w-3 h-3" /> Licensing Ledger
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5 text-left">
...
export const EditStaffDialog: React.FC<any> = ({
  ...
  useEffect(() => {
    if (staffMember && open) {
      const specialtiesString = Array.isArray(staffMember.specialties)
      ...
      methods.reset({
        ...
        compliance: {
          ...
          licenseExpiry: staffMember.compliance?.licenseExpiry
            ? parseISO(staffMember.compliance.licenseExpiry)
            : undefined,
        },
      ...
      <DialogContent className="p-0 border-none bg-background flex flex-col shadow-3xl overflow-hidden sm:max-w-2xl h-[90dvh]">
        <FormProvider {...methods}>
          <form
            ...
            className="flex flex-col flex-1 min-h-0"
          >
            <DialogHeader className="flex-shrink-0 text-left border-b bg-muted/5 p-10 pb-6">
              <div className="flex items-center gap-3 mb-2">
                <Edit className="w-5 h-5 text-primary" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground opacity-60">
                  Strategic Refinement
