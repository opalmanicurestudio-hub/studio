...
import {
  ...
  Fingerprint, Award, Heart, Percent, DollarSign,
} from 'lucide-react';
...
const addStaffSchema = z.object({
  ...
  // licenseExpiry stored as string 'yyyy-MM-dd' -- no Popover/Calendar needed
  compliance: z.object({
        ...
        <div className="p-6 bg-primary/5 rounded-[2.5rem] border-4 border-primary/10 space-y-4 shadow-inner">
          ...
          <div className="flex flex-col items-center gap-2">
            <Input
              ...
              readOnly
            />
            <p className="text-[10px] text-center text-muted-foreground uppercase font-bold tracking-tight opacity-60">Required for terminal access and overrides.</p>
          </div>
        </div>
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                ...
                <SelectContent className="rounded-xl border-2 shadow-2xl">
                  {pricingTiers.map(tier => (
                    <SelectItem key={tier.id} value={tier.id} className="font-bold uppercase text-[10px] tracking-widest">{tier.name.toUpperCase()}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            )} />
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-2 text-left">
          ...
          <Textarea id="bio" placeholder="Draft a compelling profile for guests..." {...register('bio')} className="rounded-2xl border-2 bg-muted/5 min-h-[100px]" />
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-dashed text-left">
          <div className="flex items-center justify-between px-1">
            ...
            <Button variant="ghost" size="sm" onClick={() => setIsServicesDialogOpen(true)} className="h-7 px-3 text-[9px] font-black uppercase tracking-widest text-primary border border-primary/20 rounded-lg hover:bg-primary/5">
        ...
        <div className="space-y-4 text-left">
          <div className="flex items-center justify-between px-1">
            ...
            <Button variant="ghost" size="sm" onClick={() => setIsConsentFormDialogOpen(true)} className="h-7 px-3 text-[9px] font-black uppercase tracking-widest text-primary border border-primary/20 rounded-lg hover:bg-primary/5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="commissionRate" type="number" placeholder="40" {...register('commissionRate')} className="h-12 pr-8 rounded-xl border-2 font-black text-lg text-primary shadow-inner" />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="retailCommissionRate" type="number" placeholder="10" {...register('retailCommissionRate')} className="h-12 pr-8 rounded-xl border-2 font-black text-lg text-primary shadow-inner" />
                  <Percent className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-primary opacity-40" />
                </div>
              </div>
            </div>
          ...
          {(payStructure === 'hourly' || payStructure === 'hourly_plus_commission') && (
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="hourlyRate" type="number" placeholder="25.00" {...register('hourlyRate')} className="h-14 pl-10 rounded-2xl border-2 font-black text-xl font-mono text-primary shadow-inner" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-dashed text-left">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Native date input -- works reliably on all mobile browsers inside Dialog */}
              <div className="space-y-1.5">
  ...
  useEffect(() => {
    if (open) {
      ...
      methods.reset({
        ...
        services: [], assignedFormIds: [], pin: newPin, showOnPublicPage: true,
      });
      setStep(1);
    }
      ...
      <form id="add-staff-wizard-form" onSubmit={methods.handleSubmit(onSave)} className="flex flex-col flex-1 min-h-0">
        ...
        <DialogFooter className={cn('border-t bg-background flex-shrink-0 shadow-2xl', isMobile ? 'p-4' : 'p-6 sm:p-8 pt-4')}>
          <div className="flex w-full gap-4">
            {step > 1 && (
              <Button variant="ghost" onClick={handleBack} type="button" className="flex-1 h-12 md:h-16 rounded-3xl font-black uppercase tracking-tighter text-[10px] md:text-2xl text-slate-400">
                Back
              </Button>
            )}
            <div className={cn('flex gap-3', step === 1 ? 'w-full' : 'flex-[2.5]')}>
              <Button variant="outline" onClick={() => onOpenChange(false)} type="button" className="flex-1 h-12 md:h-16 rounded-3xl font-black uppercase tracking-widest text-[10px] md:text-xl border-2">
                Cancel
              </Button>
              {step < totalSteps ? (
                <Button onClick={handleNext} type="button" className="flex-[1.5] h-12 md:h-16 font-black uppercase tracking-widest text-[10px] md:text-xl rounded-[2rem] shadow-2xl shadow-primary/30 group">
                  Continue <ArrowRight className="ml-3 w-4 h-4 md:w-8 md:h-8 transition-transform group-hover:translate-x-1" />
                </Button>
              ) : (
                <Button type="submit" className="flex-[1.5] h-12 md:h-16 font-black uppercase tracking-widest text-[10px] md:text-xl rounded-[2rem] shadow-2xl shadow-primary/30">
                  Save Provider
                </Button>
              )}
            </div>
