...
export const BookingWelcome = ({ tenant }: { tenant: Tenant | null }) => {
  ...
  const settings = tenant?.bookingPageSettings;
    ...
    <section id="welcome" className="relative py-10 md:py-20 text-center flex flex-col items-center">
      ...
      >
        <div className="inline-flex items-center gap-3 bg-primary/5 px-6 py-2 rounded-full border-2 border-primary/10 shadow-sm mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Now Booking for {currentMonth}</span>
        </div>
        
        <h2 className="text-5xl md:text-8xl font-black tracking-tighter text-slate-900 leading-[0.9] uppercase">
            {settings?.heroTitle || 'Your journey to'} <span className="text-primary italic font-serif lowercase tracking-normal">clarity</span> {settings?.heroSubtitle || 'starts here.'}
        </h2>
        
        <p className="text-lg md:text-xl text-muted-foreground font-medium leading-relaxed max-w-2xl mx-auto italic">
          "{settings?.welcomeMessage || `Expertise meets intuition. We are thrilled to welcome you to ${tenant?.name || 'our studio'}, where every detail is designed for your ultimate transformation.`}"
        </p>
