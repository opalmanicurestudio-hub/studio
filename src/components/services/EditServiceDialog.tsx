...
const RecoveryMatrix = ({ pricingTiers, currentValues, tmhr, taxBurden, staff }: { pricingTiers: PricingTier[]; currentValues: any; tmhr: number; taxBurden: number; staff: Staff[] }) => {
  ...
  const tierAnalysis = useMemo(() => [...pricingTiers].sort((a, b) => (a.rank || 0) - (b.rank || 0)).map(tier => {
        ...
        {pricingTiers.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[...pricingTiers].sort((a, b) => (a.rank || 0) - (b.rank || 0)).map(tier => <PricingTierInput key={tier.id} tier={tier} />)}
            </div>
          </div>
        )}
