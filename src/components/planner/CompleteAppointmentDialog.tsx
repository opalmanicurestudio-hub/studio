...
- `src/components/planner/CompleteAppointmentDialog.tsx`
- `src/app/(app)/clients/[id]/page.tsx`
I've made all the necessary changes to these files to implement the requested features.
1.  **Event Emitter**: I updated `src/firebase/error-emitter.ts` to include a new `'incident-reported'` event.
2.  **Notification Listener**: In `src/components/shared/AppHeader.tsx`, I added a `useEffect` hook that listens for the `incident-reported` event and dynamically adds a new notification to the UI.
3.  **Checkout Incident Logging**: In `src/components/planner/CompleteAppointmentDialog.tsx`, I updated the `handleCheckout` function. It now uses `arrayUnion` to add the new incident to the client's `intel.incidents` array in Firestore and emits the `'incident-reported'` event.
4.  **Client Page Incident Logging**: Similarly, in `src/app/(app)/clients/[id]/page.tsx`, the `handleIncidentLogged` function now also emits the `'incident-reported'` event after logging an incident directly from the client's profile.

I will now generate the changes.Excellent points on both fronts. True accountability requires clear data trails, and the financial calculations must be precise. I've implemented a series of updates to address these critical issues.

Here’s a summary of the improvements:

1.  **Inventory Ledger Update:** When a product is used in a service, the inventory ledger will now record which staff member's appointment triggered the stock correction, giving you a clear audit trail.
2.  **Accurate Staff KPIs:** I've overhauled the financial calculations on the Staff Management page and in the Staff Details sheet.
    *   **Correct Take-Home Pay:** The "Est. Take-home" value now correctly includes service wages (hourly or commission), retail commission, *and* tips.
    *   **Detailed Breakdown:** The slide-out staff dashboard provides a more detailed breakdown of earnings for clearer analysis.
3.  **Consumption KPI Fix:** I found and fixed the bug in the consumption KPI. The system now correctly logs the full appointment ID with stock corrections and accurately calculates the cost of products consumed by each staff member, giving you a true picture of their profitability.

These changes provide greater accuracy in your financial reporting and better visibility into how resources are being used. Here are the updated files.

<changes>
  <description>Improves staff accountability by linking inventory usage to specific appointments in the ledger. Also corrects and enhances staff KPI calculations, including take-home pay and product consumption, for more accurate financial reporting.</description>
  <change>
