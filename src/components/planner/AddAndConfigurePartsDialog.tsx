...
import { Search, List, Sparkles, User, Zap, Workflow, Clock, DollarSign, Check, X } from 'lucide-react';
...
export const AddAndConfigurePartsDialog: React.FC<AddAndConfigurePartsDialogProps> = ({
  ...
  const ContentContainer = isMobile ? SheetContent : DialogContent;

  const renderContent = (
    <div className="flex flex-col h-full">
      <div className={cn("p-6 pb-4 border-b bg-muted/5 flex-shrink-0 text-left", isMobile && "p-8")}>
        ...
        <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-slate-900">Add & Configure Parts</h2>
        <p className="text-xs font-bold uppercase tracking-widest opacity-60 mt-1">Select new parts and assign providers inline.</p>
      </div>
      ...
      <div className={cn("p-6 md:p-8 border-t bg-muted/5 flex-shrink-0", isMobile && "p-8")}>
  ...
  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="bottom" className="h-[95vh] p-0 border-none rounded-t-[3rem] overflow-hidden bg-background">
          {renderContent}
        </SheetContent>
      </Sheet>
    );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl p-0 border-4 rounded-[3rem] overflow-hidden shadow-3xl bg-background max-h-[90vh]">
        {renderContent}
      </DialogContent>
    </Dialog>
  );
