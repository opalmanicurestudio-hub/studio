...
const AddAppointmentForm = ({ 
    ...
    const handleSubmit = () => {
        if (!selectedClientId || !selectedService || !date) return;
...
export const AddAppointmentDialog = ({ open, onOpenChange, clients, services, onConfirm }: { open: boolean, onOpenChange: (open: boolean) => void, clients: Client[], services: Service[], onConfirm: (apt: Omit<Appointment, 'id'>) => void }) => {
  const isMobile = useIsMobile();

  const title = "New Appointment";
  const description = "Book a new appointment for a client.";
  
  const [formData, setFormData] = useState<Partial<Omit<Appointment, 'id'>>>({});

  const handleConfirm = () => {
    // This is a simplified handler. The form itself handles confirmation now.
    const formElement = document.getElementById('add-appointment-form');
    if (formElement) {
        // This is a bit of a hack to trigger form submission from outside
        formElement.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
    }
  }
  
  const FormContent = <AddAppointmentForm clients={clients} services={services} onConfirm={onConfirm} />;
    ...
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="bottom" className="h-[95dvh]">
          ...
          <form id="add-appointment-form" onSubmit={(e) => { e.preventDefault(); handleConfirm(); }}>
