...
import {
  ...
  RefreshCw, History, HeartHandshake, AlertCircle, BookMarked
} from 'lucide-react';
...
export const AppointmentDetailsSheet: React.FC<any> = ({
  ...
  const handleEscalate = async () => {
    ...
    adminsAndOwners.forEach(admin => {
      ...
      batch.set(notifRef, { id: notifRef.id, userId: admin.id, type: 'escalation', message: `URGENT ESCALATION: Service Issue for ${client?.name || 'Guest'} at ${appointment.id.slice(-6).toUpperCase()}`, link: `/pos?checkout_id=${appointment.id}`, createdAt: now, read: false });
    });
    try {
      await batch.commit();
  ...
  const financialData = useMemo(() => {
    ...
    const revenue = isCompleted ? transactions.filter((t: any) => t.appointmentId === appointment.id && t.category === 'Service Revenue').reduce((acc: any, t: any) => acc + t.amount, 0) : baseRevenue;
  ...
  const handleAddAndConfigureConfirm = (selectedAddOns: Service[], configs: any) => {
    ...
    selectedAddOns.forEach((s) => {
      ...
      if (config) { newStaffOverrides[s.id] = config.staffId; if (config.isConcurrent) newConcurrentIds.push(s.id); }
    });
    updateDocumentNonBlocking(appointmentRef, { addOnIds: selectedAddOns.map(s => s.id), checkoutState: { ...currentCheckoutState, serviceStaffOverrides: newStaffOverrides, concurrentServiceIds: Array.from(new Set(newConcurrentIds)) } });
    ...
    <ScrollArea className="flex-1 overflow-y-auto">
      <div className="space-y-8 p-5 md:p-8 pb-16">
        ...
        {appointment.status === 'confirmed' && (
          <Button onClick={() => onStartService(appointment.id)} className="w-full h-14 rounded-2xl font-black uppercase shadow-2xl shadow-primary/20" size="lg">
        ...
        <div className="flex items-center gap-4">
          <Avatar className="w-16 h-16 border-4 border-background shadow-xl rounded-[1.5rem] shrink-0">
            ...
            <AvatarFallback className="text-xl font-black bg-primary/10 text-primary uppercase">{(client?.name || 'G').substring(0, 2).toUpperCase()}</AvatarFallback>
          </Avatar>
          <div className="space-y-1 flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {client.activeMembershipId && <Badge className="h-5 px-2 rounded-full font-black uppercase text-[8px] tracking-widest bg-indigo-600 text-white border-none"><Award className="w-2.5 h-2.5 mr-1" /> Member</Badge>}
            </div>
            {isOwnerOrAdminUser && (
          ...
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="pref-notes" className="border-2 rounded-2xl overflow-hidden bg-muted/5 mb-2 shadow-inner">
              ...
              <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
                ...
                {!client.notes?.goals && !client.sensoryNeeds && !client.notes?.history && <p className="text-[9px] font-bold text-muted-foreground uppercase opacity-40 text-center py-2">No preference data archived</p>}
              </AccordionContent>
            </AccordionItem>
                  ...
                  <div className="grid gap-2">
                    {client.customFormulas.map((f: any) => (
                      <div key={f.id} className="p-3 rounded-xl bg-white border-2 border-transparent hover:border-primary/10 transition-all flex justify-between items-center shadow-sm">
                        <div className="min-w-0"><span className="text-[10px] font-black uppercase tracking-tight truncate block">{f.name}</span><span className="text-[8px] font-bold text-muted-foreground uppercase opacity-60">{format(safeDate(f.date), 'MMM d, yyyy')}</span></div>
        ...
        <div className="space-y-3 pt-4 border-t border-dashed">
          ...
          <div className={cn("flex flex-col gap-4 p-5 rounded-[2rem] border-4 transition-all shadow-xl", appointment.isEscalated ? "bg-destructive text-white border-destructive shadow-destructive/20" : "border-destructive/20 bg-destructive/[0.02] shadow-destructive/5")}>
            <div className="flex items-center justify-between gap-4">
              ...
              <Button variant={appointment.isEscalated ? "secondary" : "destructive"} size="icon" disabled={isEscalating} onClick={appointment.isEscalated ? undefined : handleEscalate} className={cn("h-14 w-14 rounded-2xl shadow-xl", !appointment.isEscalated && "shadow-destructive/20 animate-pulse")}>
        ...
        {appointment.inspirationPhotoUrl && (
          <div className="space-y-3 pt-4 border-t border-dashed">
            ...
            <div className="relative aspect-video w-full rounded-[2rem] overflow-hidden border-2 border-primary/10 bg-muted/5 group shadow-inner cursor-zoom-in" onClick={() => setExpandedImage(appointment.inspirationPhotoUrl)}>
              ...
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Maximize2 className="w-8 h-8 text-white" /></div>
              <div className="absolute top-4 right-4"><Badge className="bg-primary/90 backdrop-blur-md text-white border-none font-black text-[8px] uppercase h-6 px-3 shadow-xl">Guest Choice</Badge></div>
            </div>
          </div>
        )}
        ...
        {financialData && financialData.adjustmentCharge > 0 && (
          <Alert className="border-2 border-primary/20 bg-primary/[0.01] rounded-2xl p-5 shadow-sm">
            ...
            <AlertDescription className="text-[10px] font-bold leading-relaxed opacity-80 uppercase">This session includes <strong>${financialData.adjustmentCharge.toFixed(2)}</strong> in adjustments to be collected at checkout.</AlertDescription>
          </Alert>
        )}
        {safeNumber(client.outstandingBalance) > 0 && (
          <Alert variant="destructive" className="bg-destructive/5 border-destructive/20 border-2 rounded-2xl p-5 shadow-sm">
            ...
            <AlertDescription className="text-[10px] font-bold leading-relaxed opacity-80 uppercase">Client owes <strong>${Number(client.outstandingBalance).toFixed(2)}</strong>. Settle at checkout.</AlertDescription>
          </Alert>
        )}
            ...
            <CardContent className="p-4 space-y-4">
              <div className="flex justify-between items-start gap-4">
                <div className="space-y-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[9px] font-bold text-muted-foreground uppercase tracking-widest"><Clock className="w-2.5 h-2.5" /> {service.duration}m</div>
                  <div className="flex items-center gap-2 mt-2 pt-2 border-t border-dashed border-primary/10">
                    <Avatar className="h-5 w-5 border shadow-sm"><AvatarImage src={mainStaffMember?.avatarUrl} className="object-cover" /><AvatarFallback className="text-[8px] font-black bg-primary/10 text-primary">{(mainStaffMember?.name || 'S')[0]}</AvatarFallback></Avatar>
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent
          ...
          className={cn(
            ...
            isMobile
              ? 'h-[92dvh] rounded-t-[2.5rem] w-full'
              : 'sm:max-w-xl'
          )}
        >
      ...
      {appointment.inspirationPhotoUrl && isMarkupOpen && (
        <ImageMarkupDialog open={isMarkupOpen} onOpenChange={setIsMarkupOpen} imageUrl={appointment.inspirationPhotoUrl} onSave={handleMarkupSave} title={`Mapping for ${client.name}`} />
      )}
