        ...
        <div className="flex flex-col h-full">
            <div className="mb-8 flex-shrink-0">
                ...
                <div className="flex gap-2 mt-2">
                    <Dialog open={isPayerDialogOpen} onOpenChange={setIsPayerDialogOpen}>
                        <DialogTrigger asChild>
                            ...
                            >
                                ...
                                ) : <span className="opacity-40">{isGroupCheckout ? "Select Account..." : "Search Payer..."}</span>}
                    ...
                    <Button variant="outline" size="icon" className="h-14 w-14 rounded-2xl border-2 shadow-sm shrink-0" onClick={onScanClick}><QrCode className="w-6 h-6" /></Button>
                </div>
            </div>

            <ScrollArea className="flex-1 min-h-0 -mx-2 px-2">
                <div className={cn("space-y-10 pb-10", isMobile && "px-2 pt-2")}>
                    {selectedClient && (selectedClient.outstandingBalance || 0) > 0 && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-[2rem] border-4 border-destructive bg-destructive/5 text-destructive shadow-2xl shadow-destructive/5 space-y-4">
                            <div className="flex items-center gap-3">
                        ...
                        {isCartEmpty ? (
                            <div className="py-16 text-center border-4 border-dashed rounded-[3rem] opacity-30 flex flex-col items-center gap-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-bold uppercase tracking-tight">Scan a ticket or select retail items</p>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {appointmentsData.map((data: any) => {
                                    ...
                                    return (
                                        <Card key={data.appointment.id} className={cn("overflow-hidden rounded-[2rem] border-2 shadow-sm transition-all", isRedeemed ? "border-primary bg-primary/[0.03] shadow-lg" : "border-border/50 bg-muted/5")}>
                                ...
                                {cart.map((item: any) => (
                                    <div key={item.id} className="p-4 rounded-3xl bg-muted/20 border-2 border-transparent hover:border-primary/10 transition-all flex items-center gap-4 group">
                                        <div className="flex-1 min-w-0 text-left">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="flex items-center bg-background rounded-xl border-2 h-9 px-1 shadow-sm">
                                                <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg hover:bg-primary/5" onClick={() => handleUpdateQuantity(item.id, item.quantity - 1)}><Minus className="h-3 w-3"/></Button>
                                                <span className="w-8 text-center text-xs font-black">{item.quantity}</span>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg hover:bg-primary/5" onClick={() => handleUpdateQuantity(item.id, item.quantity + 1)}><Plus className="h-3 w-3"/></Button>
                                            </div>
                                            <p className="font-black font-mono text-sm tracking-tighter w-16 text-right text-slate-900">${(item.price * item.quantity).toFixed(2)}</p>
                                        </div>
                                ...
                                {Array.from(appliedAdjustments).map(id => {
                                    ...
                                    return (
                                        <div key={id} className="p-4 rounded-[2rem] border-2 border-destructive/20 bg-destructive/[0.02] flex items-center gap-4 animate-in fade-in slide-in-from-left-2">
                                            <div className="p-2.5 bg-destructive/10 rounded-2xl shadow-inner"><Wallet className="w-5 h-5 text-destructive" /></div>
                                            <div className="flex-1 min-w-0 text-left">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-3">
                            <div className="relative flex-1">
                                ...
                                <Input placeholder="ENTER PROMO..." value={promoCodeInput} onChange={e => setPromoCodeInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleApplyDiscount(promoCodeInput)} className="pl-10 h-12 rounded-2xl border-2 font-black uppercase text-xs tracking-widest focus-visible:ring-primary/20 bg-muted/5 shadow-inner" />
                            </div>
                            <Button variant="outline" size="icon" className="h-12 w-12 rounded-2xl border-2 shadow-sm shrink-0" onClick={() => setIsDiscountBrowserOpen(true)}><Users className="w-5 h-5" /></Button>
                        </div>

                        {appliedDiscountCodes.length > 0 && (
                            <div className="space-y-2">
                                {appliedDiscountCodes.map((code: string) => (
                                    <div key={code} className="p-3 rounded-2xl bg-primary/10 border-2 border-primary/20 flex items-center justify-between animate-in zoom-in-95">
                                        <div className="flex items-center gap-2 px-1">
                                            <CheckCircle className="h-4 w-4 text-primary" />
                                            <p className="text-xs font-black uppercase tracking-widest text-primary">{code}</p>
                                        </div>
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-primary hover:bg-primary/10 rounded-xl" onClick={() => handleRemoveDiscount(code)}><X className="h-4 w-4" /></Button>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {suggestedDiscounts.length > 0 && (
                            <div className="space-y-3 pt-2">
                                ...
                                {suggestedDiscounts.map(d => (
                                    <Button key={d.id} variant="outline" className="w-full justify-between h-auto py-4 px-5 border-amber-500/20 bg-amber-500/[0.03] hover:bg-amber-500/10 border-2 rounded-[1.5rem] group transition-all" onClick={() => handleApplyDiscount(d.code)}>
                ...
                <div className="space-y-2 text-sm px-1">
                    ...
                    <div className="flex justify-between items-center py-2">
                        ...
                        <div className="relative w-36">
                            ...
                            <Input type="number" value={tipAmount || ''} onChange={(e) => setTipAmount(parseFloat(e.target.value) || 0)} className="h-11 text-right pr-4 pl-10 font-black text-xl border-2 rounded-2xl shadow-inner focus-visible:ring-primary/20 bg-muted/5" placeholder="0.00" />
                        </div>
                    </div>
                    {allInvolvedStaff.length > 1 && (
                        <div className="p-4 rounded-[1.5rem] border-2 bg-muted/10 space-y-3">
                            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2 opacity-60"><Users className="w-3 h-3" /> Distribution Matrix</p>
                            {allInvolvedStaff.map((member: any) => (
                                <div key={member.id} className="flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <Avatar className="h-6 w-6 border-2 border-white shadow-sm rounded-lg"><AvatarImage src={member.avatarUrl} className="object-cover" /><AvatarFallback className="font-black text-[8px]">{(member.name || 'S')[0]}</AvatarFallback></Avatar>
                                        <span className="text-[10px] font-black uppercase tracking-tight truncate text-slate-700">{member.name.split(' ')[0]}</span>
                                    </div>
                                    <div className="relative w-24">
                                        <DollarSign className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
                                        <Input type="number" value={tipAllocations[member.id] || ''} onChange={(e) => setTipAllocations({...tipAllocations, [member.id]: parseFloat(e.target.value) || 0})} className="h-8 text-right text-xs pr-2 pl-6 font-bold rounded-xl border-primary/10 focus-visible:ring-primary/20" />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex justify-between items-baseline font-black text-3xl md:text-4xl text-primary tracking-tighter px-1 pb-2">
                        <div className="space-y-0.5">
                            <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground opacity-60">Session Total</p>
                            <p className="text-[9px] font-bold uppercase text-primary/40">INC. TAX & TIPS</p>
                        </div>
                        <p className="font-mono text-4xl">${total.toFixed(2)}</p>
                    </div>
                </div>
                
                <div className="mt-6 space-y-4 pb-10 px-1">
                    <RadioGroup value={paymentTab} onValueChange={setPaymentTab} className="grid grid-cols-3 gap-3">
                        <div><RadioGroupItem value="cash" id="hub-pay-cash" className="peer sr-only" /><RadioLabel htmlFor="hub-pay-cash" className="flex flex-col items-center justify-center rounded-2xl border-2 border-muted bg-white p-2 text-[10px] font-black uppercase tracking-widest hover:bg-accent peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/[0.03] peer-data-[state=checked]:text-primary transition-all cursor-pointer h-20 shadow-sm"><Banknote className="mb-1.5 h-6 w-6 opacity-40" />Cash</RadioLabel></div>
                        <div><RadioGroupItem value="card" id="hub-pay-card" className="peer sr-only" /><RadioLabel htmlFor="hub-pay-card" className="flex flex-col items-center justify-center rounded-2xl border-2 border-muted bg-white p-2 text-[10px] font-black uppercase tracking-widest hover:bg-accent peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/[0.03] peer-data-[state=checked]:text-primary transition-all cursor-pointer h-20 shadow-sm"><CreditCard className="mb-1.5 h-6 w-6 opacity-40" />Card</RadioLabel></div>
                        <div><RadioGroupItem value="scan" id="hub-pay-scan" className="peer sr-only" /><RadioLabel htmlFor="hub-pay-scan" className="flex flex-col items-center justify-center rounded-2xl border-2 border-muted bg-white p-2 text-[10px] font-black uppercase tracking-widest hover:bg-accent peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/[0.03] peer-data-[state=checked]:text-primary transition-all cursor-pointer h-20 shadow-sm"><Scan className="mb-1.5 h-6 w-6 opacity-40" />Scan</RadioLabel></div>
                    </RadioGroup>
                        ...
                        <div className="space-y-4 pt-2 animate-in slide-in-from-top-4 duration-500">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5 text-left">
                                    ...
                                    <div className="relative">
                                        <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground font-black" />
                                        <Input type="number" value={amountTendered || ''} onChange={(e) => setAmountTendered(parseFloat(e.target.value) || 0)} className="pl-10 h-14 font-black text-2xl border-2 rounded-2xl shadow-inner bg-muted/5 focus-visible:ring-primary/20" />
                                    </div>
                                </div>
                                {amountTendered > total && (
                                    <div className="space-y-1.5 text-left">
                                        <Label className="text-[10px] uppercase font-black tracking-widest text-green-600 ml-2">Change Due</Label>
                                        <div className="h-14 flex items-center justify-center bg-green-500/10 border-2 border-green-500/20 rounded-2xl shadow-sm">
                                            <p className="font-black text-2xl text-green-600 font-mono tracking-tighter">-${(amountTendered - total).toFixed(2)}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                                {quickTenderOptions.map(val => (
                                    <Button key={val} variant="outline" size="sm" className="flex-1 font-black h-11 rounded-xl text-xs shrink-0 border-2 bg-background hover:bg-primary/5" onClick={() => setAmountTendered(val)}>${val}</Button>
                                ))}
                                <Button variant="outline" size="sm" className="flex-1 font-black h-11 rounded-xl border-2 border-primary text-primary text-xs shrink-0 hover:bg-primary/5" onClick={() => setAmountTendered(total)}>EXACT AMOUNT</Button>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="pt-2">
                        <Button 
                            className="w-full h-16 text-xl font-black rounded-[2rem] shadow-2xl shadow-primary/30 transition-all hover:scale-[1.02] active:scale-95 uppercase tracking-tight" 
                        ...
                        >
                            {isSubmitting ? <Loader className="animate-spin h-7 w-7" /> : (total <= 0 ? 'FINALIZE FREE SESSION' : `COLLECT $${total.toFixed(2)}`)}
                        </Button>
                    </div>
                </div>
            </div>
